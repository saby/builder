...
DepGraph.prototype._visitNode = function visitNode(taskParameters, maxLvl, name) {
   ...
   if (node) {
      if (node.mark > 0) {
         if (node.mark === 1) {
            const currentNodeName = node.path.replace(/(\.min)?\.(js|wml|tmpl|xhtml)$/, '');
            const cycleDepIndex = this._path.findIndex(element => element === currentNodeName);
            const cyclePath = this._path.slice(cycleDepIndex, this._path.length);

            if (!this._cyclePaths.hasOwnProperty(currentNodeName)) {
               const moduleInfo = taskParameters.config.getModuleInfoByName(currentNodeName);

               this._cyclePaths[currentNodeName] = cyclePath.join('-->');
               logger.warning({
                  message: `Cycle dependency detected: ${this._cyclePaths[node.module]}`,
                  moduleInfo
               });
            }
         }
      ...
      node.mark = 1;
      (links[name] || []).forEach(visitNode.bind(this, taskParameters, maxLvl));
      node.mark = 2;
      node.weight = this._n++;
      ...
      visitNode.call(this, taskParameters, maxLvl, name.replace('browser!', ''));
   } else if (name && name.indexOf('optional!') > -1) {
      visitNode.call(this, taskParameters, maxLvl, name.replace('optional!', ''));
   }
 ...
 * @returns {Array}
 */
DepGraph.prototype.getLoadOrder = function getLoadOrder(taskParameters, startNodes, maxLevel = Infinity) {
   const self = this;
   ...
   startNodes.forEach(this._visitNode.bind(this, taskParameters, maxLevel));
