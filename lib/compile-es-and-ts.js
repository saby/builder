...
const normalizeDependencies = require('./espree/modify-amd-dependencies');

const isRoutesFile = filePath => /\.routes\.tsx?$/.test(filePath);
...
function getTranspileOptions(
   ...
   if (needToRemoveModuleParam) {
      currentCompilerOptions.module = 'commonjs';
   }
...
function getExtraMetaAboutModule(text, moduleName, relativePath) {
   ...
   const defineRegEx = new RegExp(`define\\(\\s*['"]${moduleName.split(/\\|\//).shift()}`);
   const removeModuleParam = defineRegEx.test(text) || isRoutesFile(relativePath);
...
function compileEsAndTs(
   ...
   const moduleName = transliterate(relativePath).replace(/(\.meta)?\.(ts|tsx)$/, '');
   const result = {};
   ...
   Object.keys(compilerOptions).forEach((currentBuildMode) => {
      ...
      if (moduleName === 'RequireJsLoader/config') {
         // если не удалить данную конструкцию, при исполнении кода будет попытка определить
         // в global scope для переменной exports поле __esModule, что приведёт к ошибке
         transpileResult.outputText = transpileResult.outputText.replace('Object.defineProperty(exports, "__esModule", { value: true });', '');
      }
