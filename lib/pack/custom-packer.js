...
async function saveBundlesForEachModule(taskParameters, applicationRoot, result, modulesForPatch) {
   ...
   await pMap(
      ...
      async(currentBundle) => {
         ...
         if (['Superbundles', 'RetailOffline'].includes(intModuleName)) {
      ...
      async(currentModule) => {
         ...
         if (modulesForPatch.length === 0 || modulesForPatch.includes(currentModule)) {
            ...
            if (await fs.pathExists(currentBundlesRoutePath)) {
               ...
               // interface module
               await storePackageMap(taskParameters, applicationRoot, currentModule, currentBundlesRoute);
               await updateContents(taskParameters, applicationRoot, currentModule);
            }
