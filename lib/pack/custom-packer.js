...
const { convertModulesToBeLazy } = require('../esprima/convert-to-lazy-module');
...
async function customPackCSS(config, files, root, relativePackagePath) {
   ...
   files.forEach((currentCss) => {
      const normalizedCss = currentCss.replace(/(_ie|\.rtl)(\.min)?.css$/, '$2.css');
   ...
   if (config.sources) {
      ...
      cssResults.ie = resultsForIE.join('\n');
   }
...
async function generateCustomPackage(
   ...
   if (cssModulesFromOrderQueue.length > 0) {
      ...
      if (taskParameters.config.sources) {
         taskParameters.addVersionedModule(moduleName, `${packagePath.replace(/(\.min)?$/, '_ie$1')}.css`);
         taskParameters.addCdnModule(moduleName, `${packagePath.replace(/(\.min)?$/, '_ie$1')}.css`);
      }
   ...
   if (cssModulesFromOrderQueue.length > 0) {
      ...
      if (taskParameters.config.sources) {
         taskParameters.addFileToCopy(moduleName, `${relativePackagePath.replace(/(\.min)?$/, '_ie$1')}.css`);
      }
      if (taskParameters.config.buildRtl) {
...
async function generateAllCustomPackages(configs, taskParameters, depsTree, results, root) {
   ...
   await pMap(
      configsArray,
      async(currentConfig) => {
         const moduleName = removeLeadingSlashes(currentConfig.path).split('/').shift();
 ...
 * @returns {Promise<void>}
 */
async function splitIntersectsByUiModuleName(taskParameters, modulesForPatch, root, intersects) {
   ...
   await pMap(
      ...
      async(currentEntry) => {
         ...
         const moduleInfo = taskParameters.config.getModuleInfoByName(currentUiModuleName);
         if (moduleInfo) {
            moduleInfo.addFileHash('.builder/customPackIntersects.json', helpers.getFileHash(JSON.stringify(sortedIntersects), true));
         }
      },
      {
         concurrency: 10
 ...
 */
async function setSuperBundle(taskParameters, configs, root, modulesForPatch) {
