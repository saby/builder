...
async function writeCustomPackage(
   ...
   if (packageConfig.isPrivatePackage) {
      ...
      taskParameters.addFilesToCopy(packageConfig.moduleName, [`${prettyRelativePackagePath}.private`]);
   }
   ...
   taskParameters.addFilesToCopy(packageConfig.moduleName, [`${prettyRelativePackagePath}.lockfile`]);
...
function getCustomCssPath(packagePath) {
   return packagePath.replace(/(\.package)?(\.min)?\.js$/, '');
}
...
async function generateCustomPackage(
   ...
   const
      ...
      { resourcesUrl, countries } = taskParameters.config,
      regionNodes = taskParameters.cache.getRegionNodes(),
      result = {
         bundles: {},
         bundlesRoute: {},
         optionalBundles: {},
   ...
   orderQueue = packHelpers.getOrderQueue(
      ...
      if (node.fullName.includes('css!')) {
         ...
         if (
            ...
            regionNodes &&
            regionNodes.hasOwnProperty(node.fullName)
         ) {
            if (!packageConfig.regionCss) {
               packageConfig.regionCss = [];
            }
            packageConfig.regionCss.push(node.fullName);
         }
   ...
   if (cssModulesFromOrderQueue.length > 0) {
      taskParameters.addVersionedModules(moduleName, [`${packagePath}.css`]);

      if (taskParameters.config.buildRtl) {
         taskParameters.addVersionedModules(moduleName, [`${packagePath.replace(/(\.min)?$/, '.rtl$1')}.css`]);
      }
   }

   taskParameters.addFilesToCopy(moduleName, [`${relativePackagePath}.js`]);
   if (cssModulesFromOrderQueue.length > 0) {
      taskParameters.addFilesToCopy(moduleName, [`${relativePackagePath}.css`]);

      if (taskParameters.config.buildRtl) {
         taskParameters.addFilesToCopy(moduleName, [`${relativePackagePath.replace(/(\.min)?$/, '.rtl$1')}.css`]);
      }
   }
   ...
   if (cssModulesFromOrderQueue.css.length > 0) {
      ...
      let cssOutputFile;
      if (packageConfig.regionCss && packageConfig.regionCss.length > 0) {
         const foreignCountries = countries.filter(
            region => builderConstants.CURRENCY_GLYPH_MAP.hasOwnProperty(region)
         );
         const promises = [];
         foreignCountries.forEach((region) => {
            let regionPackage = cssRes.common;
            packageConfig.regionCss.forEach((currentCss) => {
               const currentReplaceNodes = regionNodes[currentCss][region];
               Object.keys(currentReplaceNodes).forEach((currentKey) => {
                  regionPackage = regionPackage.replace(new RegExp(currentKey), currentReplaceNodes[currentKey]);
               });
            });

            const regionCssPackagePath = getCustomCssPath(
               packHelpers.getOutputFile(
                  packageConfig,
                  path.join(`${root}_${region}`, application),
                  depsTree
               )
            );
            cssOutputFile = `${regionCssPackagePath}${cssExtIncludesPackage ? '.package' : ''}.min.css`;

            promises.push(fs.outputFile(cssOutputFile, regionPackage));
         });
         await Promise.all(promises);
      }

      cssOutputFile = `${pathToCustomCSS}${cssExtIncludesPackage ? '.package' : ''}.min.css`;
      ...
      cssExternalModuleUsageMessages.forEach(message => logger.error({
         message,
         filePath: path.relative(packageConfig.moduleInfo.path, packageConfig.path),
...
async function processContentsMeta(
   jsonToWrite,
   applicationRoot,
   intModuleName,
   contentsMetaName,
   ...
   const contentsPath = path.normalize(path.join(applicationRoot, intModuleName, `${contentsName}.json`));
   const minContentsPath = path.normalize(path.join(applicationRoot, intModuleName, `${contentsName}.min.json`));
   if (await fs.pathExists(contentsPath)) {
      ...
      if (await fs.pathExists(minContentsPath)) {
         jsonToWrite[minContentsPath] = jsonToWrite[minContentsPath] || await fs.readJson(minContentsPath);
         jsonToWrite[minContentsPath].modules[intModuleName][contentsMetaName] = true;
      }
...
async function saveBundlesForEachModule(taskParameters, applicationRoot, result) {
   ...
   /**
    * don't save bundlesRoute if sources flag disabled. There is no need in usage
   ...
   const processBundleMeta = async(currentBundle, bundleMetaName, contentsMetaName, saveBundlesInOutput) => {
      ...
      taskParameters.addFilesToCopy(intModuleName, [`${bundleMetaName}.json`]);
      if (bundleMetaName === 'optionalBundles') {
         taskParameters.addFilesToCopy(intModuleName, [`${bundleMetaName}.min.js`]);
      }
   };
   ...
   await pMap(
      ...
      async(currentBundle) => {
         ...
         // обновлять contents нужно только в случае, если в данном интерфейсном модуле
         // был пересобран хотя бы 1 кастомный пакет
         if (result.modulesWithGeneratedPackages.has(intModuleName)) {
   ...
   await pMap(
      ...
      async(currentBundle) => {
         ...
         // обновлять contents нужно только в случае, если в данном интерфейсном модуле
         // был пересобран хотя бы 1 опциональный кастомный пакет
         if (result.modulesWithGeneratedPackages.has(intModuleName)) {
            await processBundleMeta(
               currentBundle,
               'optionalBundles',
   ...
   await pMap(Object.keys(jsonToWrite), async(key) => {
      ...
      if (Object.keys(jsonToWrite[key]).length > 0) {
         ...
         // обновляем мета-данные кастомной паковки только если были пересобраны кастомные пакеты или для одного
         // из модулей поменялись мета-данные о библиотеках(в bundlesRoute также сохраняется инфа о библиотеках для
         // сервиса представлений)
         if (result.modulesWithGeneratedPackages.has(moduleName) || taskParameters.librariesMeta[moduleName]) {
            if (fileName === 'bundlesRoute.json') {
               ...
               taskParameters.addFilesToCopy(moduleName, [fileName]);

               // don't save bundlesRoute in desktop application, it's only needed to save packageMap.
...
async function storePackageMap(taskParameters, applicationRoot, currentModule, currentBundlesRoute) {
   ...
   taskParameters.addFilesToCopy(currentModule, ['packageMap.json', 'packageMap.json.js', 'packageMap.json.min.js']);
}


async function getLibrariesMeta(taskParameters, applicationRoot, currentModule) {
   if (taskParameters.librariesMeta[currentModule]) {
      return taskParameters.librariesMeta[currentModule];
   }

   const librariesMetaPath = path.join(applicationRoot, currentModule, '.builder/libraries.json');

   if (await fs.pathExists(librariesMetaPath)) {
      const result = await fs.readJson(librariesMetaPath);
      return result;
   }
...
async function saveModuleCustomPackResults(taskParameters, result, applicationRoot) {
   ...
   const modulesWithLibrariesMetaToUpdate = [...result.modulesWithGeneratedPackages];

   // в librariesMeta в текущем билде могут содержаться ещё дополнительные модули, которые
   // следует обработать и прокинуть в bundlesRoute, например когда пакет в модуле не
   // пересобирался, но пересобирались библиотеки и поменялась мета библиотек
   Object.keys(taskParameters.librariesMeta).forEach((currentModule) => {
      if (!modulesWithLibrariesMetaToUpdate.includes(currentModule)) {
         modulesWithLibrariesMetaToUpdate.push(currentModule);
      }
   });
    ...
    */
   await pMap(
      modulesWithLibrariesMetaToUpdate,
      async(currentModule) => {
         const currentLibraries = await getLibrariesMeta(taskParameters, applicationRoot, currentModule);

         // Skip saving of libraries meta into bundlesRoute if there is nothing to save
         if (currentLibraries && currentLibraries.length > 0) {
            ...
            currentLibraries.forEach((currentLibrary) => {
               // don't write libraries into bundlesRoute meta if packed into custom package
            ...
            taskParameters.addFilesToCopy(currentModule, ['bundlesRoute.json']);
         }
      },
      {
         concurrency: 10
...
async function compileCurrentPackage(taskParameters, depsTree, currentConfig, results, root) {
   ...
   try {
      ...
      logger.error({
         ...
         error: err,
         filePath: path.relative(currentConfig.moduleInfo.path, currentConfig.path)
      });
   }
...
async function generateAllCustomPackages(configs, taskParameters, depsTree, results, root) {
   ...
   await pMap(
      configsArray,
      async(currentConfig) => {
         if (!currentConfig.skipCustomPack) {
            await compileCurrentPackage(taskParameters, depsTree, currentConfig, results, root);

            if (currentConfig.moduleInfo) {
               results.modulesWithGeneratedPackages.add(currentConfig.moduleInfo.outputName);
            }
         }
      },
      {
   ...
   await pMap(
      ...
      async(currentSuperBundle) => {
         ...
         currentSuperBundle.includePackages.forEach((currentPackagePath) => {
            ...
            if (neededPackage) {
               if (currentSuperBundle.skipCustomPack && !neededPackage.skipCustomPack) {
                  logger.debug(
                     `Module "${neededPackage.moduleInfo.name}" has changes and his package "${neededPackage.path}-${neededPackage.output}" ` +
                     `is a part of superbundle "${currentSuperBundle.path}-${currentSuperBundle.output}". This superbundle will be rebuilt`
                  );
                  currentSuperBundle.skipCustomPack = false;
               }
         ...
         taskParameters.addFilesToCopy(currentModuleName, [relativePath]);
      },
      {
         concurrency: 50
      }
