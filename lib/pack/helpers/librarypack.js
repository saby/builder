...
const {
   ...
   isPrivate
} = require('../../esprima/library');
const { descendingSort, formatErrorMessage } = require('../../helpers');
...
async function readModuleAndGetParamsNames(
   ...
   if (!moduleData.text) {
      ...
      if (
         (extension === 'js' && await fs.pathExists(sourcePath)) ||
      ...
      //
      // P.S. Нельзя применять эту схему для tsx файлов! Поскольку в дебаге и релизе для tsx
      // используются совершенно разные react-библиотеки и генерится разный код, он не является
      // обратно совместимым и не будет работать, остаётся только прочитать .min.js версию файла.
      // TODO перейти при сборке патчей на общую схему сборки по гитовым изменениям,
      //  чтобы избежать возникновения подобных конфликтов, будет реализовано в рамках проекта.
      //  https://online.sbis.ru/opendoc.html?guid=8fe46b08-c432-4f91-9a7c-b07bd2c0a293&client=3
      //
      if (!generateUMD && extension !== 'tsx' && modulePath.endsWith('.min.js')) {
   ...
   let moduleAst;
   try {
      moduleAst = parse(moduleData.text);
   } catch (error) {
      const message = `An error occurred while parsing compiled private dependency "${result.modulePath}".`;
      throw new Error(formatErrorMessage(message, error, moduleData.text));
   }
   ...
   result.dependencies.forEach((dependency) => {
      ...
      const needToPushToLibraryDeps = !libraryDependenciesMeta.hasOwnProperty(dependency);
