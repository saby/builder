...
const parseJsComponent = require('../../parse-js-component');
const modifyWithTailwind = require('../../tailwind/modify');
...
async function readModuleAndGetParamsNames(
   ...
   if (!moduleData.text) {
      ...
      // дебажная версия production-кода tsx файла хранится в кеше tsc, поэтому нам нужно достать её оттуда
      // и прогнать через AMD-парсер, после чего данный код готов к паковке в библиотеке. Это правило не
      // распространяется на локальные стенды, где tsc compiler не запускается ввиду своей трудоёмкости и
      // модули компилятся через transpileModule
      let isRawTscCompiledFile = false;
      if (!generateUMD && modulePath.endsWith('.min.js')) {
         if (extension === 'tsx') {
            if (typescriptOutputDir) {
               isRawTscCompiledFile = true;
               modulePath = modulePath
                  .replace(outputRoot, typescriptOutputDir)
                  .replace('.min.js', '.js');
            }
         } else {
      ...
      try {
         if (isRawTscCompiledFile) {
            const tscCompiledText = await fs.readFile(modulePath, 'utf8');

            try {
               let parsedText = tscCompiledText;
               if (tailwindInfo) {
                  parsedText = modifyWithTailwind(parsedText, tailwindInfo, ESVersion);
               }

               // прочитанный код ещё не готов к использованию, он содержит анонимный дефайн и ненормализованные
               // зависимости, прогоняем его сперва через наш парсер
               parsedText = await parseJsComponent(
                  parsedText,
                  {
                     generateUMD,
                     filePath: normalizedCurrentDependency,
                     isCompiledFromTsc: true,
                     ESVersion
                  }
               );
               moduleData.text = parsedText.amdContent;
            } catch (error) {
               throw new Error(`An error occurred during parsing tsc compiled file "${dependencyWithoutPlugins}". Error: ${error}`);
            }
         } else {
 ...
 * @returns {*}
 */
function sortPrivateModulesByDependencies(privateDependencies, libraryName) {
   ...
   const result = privateDependencies.sort(depsSortingFunc);

   if (libraryName === 'JitsiConference/app-react') {
      logger.info({
         message: `сортировка приватных зависимостей выполнена за ${(Date.now() - startDate) / 1000} секунд`,
