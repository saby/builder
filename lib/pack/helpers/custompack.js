...
function getOutputFile(packageConfig, applicationRoot, depsTree) {
   ...
   return path.join(path.normalize(applicationRoot), outputFile);
}
...
function findAllModules(dg, modules) {
   ...
   return nodes.filter(node => regexp && regexp.test(node));
}
 ...
 * @return {Array}
 */
function getOrderQueue(taskParameters, depsTree, cfg, excludedCSS, applicationRoot) {
   let excludeCoreReg;
   ...
   const orderQueue = depsTree.getLoadOrder(taskParameters, modules);
...
function getBundlePath(currentOutput, applicationRoot) {
   const result = currentOutput.replace(toPosix(applicationRoot), '');
   return toPosix(result).replace(/\.js/g, '');
}
...
async function generateBundle(orderQueue, cssModulesFromOrderQueue) {
   ...
   await pMap(
      ...
      async(node) => {
         ...
         !(await fs.pathExists(modulePath.replace(moduleExtReg, '.min.js.private')));
         ...
         if (node.amd) {
            ...
            // RequireJsLoader/plugins/* namespace should be added into bundles list also
            // with old requirejs name.
            // e.g. "RequireJsLoader/plugins/optional" should be added also with a name
            // "optional"
            if (node.fullName.startsWith('RequireJsLoader/plugins/')) {
               bundle.push(path.basename(node.fullName));
            }
   ...
   for (let i = 0; i < currentBundle.length; i++) {
      if (currentBundle[i].indexOf('css!') === 0) {
         if (!excludedCSS.hasOwnProperty(currentBundle[i])) {
            const pathToPackage = `${toPosix(rebasedBundlePath)}` +
...
function parseCssNodeName(nodeName) {
   const partsOfCss = nodeName.split('/');
   const firstPart = partsOfCss.shift();
   return {
      moduleName: firstPart.replace('css!', ''),
      remainingCss: partsOfCss.join('/')
   };
}

function getDefinesForCss(themedModulesMap, cssModules, linkHref, packageSuffix, ieSuffix) {
   let result = '';
   helpers.descendingSort(cssModules, 'fullName').forEach((module) => {
      const cssToPack = [module.fullName];
      const currentCssMeta = parseCssNodeName(module.fullName);
      if (themedModulesMap.has(currentCssMeta.moduleName)) {
         const originCssName = `${themedModulesMap.get(currentCssMeta.moduleName)}/${currentCssMeta.remainingCss}`;
         cssToPack.push(`css!${originCssName}`);
         cssToPack.push(`css!theme?${originCssName}`);
      }
      cssToPack.forEach((currentCss) => {
         result += `define('${currentCss}',['css!${linkHref}` +
            `${module.currentTheme ? `_${module.currentTheme}` : ''}${packageSuffix ? `.package${ieSuffix}` : ieSuffix}'],'');`;
      });
   });
   return result;
}

function getCSSBlock(taskParameters, cssModules, linkHref, packageSuffix) {
   const { sources, buildRtl } = taskParameters.config;
   let result = '';
   if (sources) {
      let packagePostfix = '';
      ...
      if (!packagePostfix) {
         return getDefinesForCss(taskParameters.themedModulesMap, cssModules, linkHref, packageSuffix, '');
      }

      result += getDefinesForCss(taskParameters.themedModulesMap, cssModules, linkHref, packageSuffix, packagePostfix);
      result += '} else {';
      result += getDefinesForCss(taskParameters.themedModulesMap, cssModules, linkHref, packageSuffix, '');
      result += '}';
 ...
 * @returns {string}
 */
function generateLinkForCss(taskParameters, cssModules, packagePath) {
   ...
   if (taskParameters.config.buildRtl) {
      result += 'var isRtl = typeof document !== \'undefined\' && document.cookie && (document.cookie.indexOf(\'lang=ar\') !== -1 || document.cookie.indexOf(\'lang=he\') !== -1);';
   }

   result += getCSSBlock(taskParameters, cssModules, linkHref, packageSuffix);
 ...
 * @param {String} idProperty - meta type(bundles, bundlesRoute)
 */
