...
function trimCallStack(stack, length) {
   if (stack) {
...
function getFontBase64ByFile(filePath) {
   try {
      ...
      logger.info(`Could not process ${filePath} font. Error: ${error.message} Stack: ${trimCallStack(error.stack, 5)}`);
   }
   return '';
}
...
async function getFontBase64ByUrl(urlPath) {
   try {
      ...
      logger.info(`Could not process ${urlPath} font. Error: ${error.message} Stack: ${trimCallStack(error.stack, 5)}`);
   }
   return '';
}
...
function checkModuleInDependencies(link, versionModuleName, currentModuleName, moduleInfo) {
   if (versionModuleName !== currentModuleName && !moduleInfo.fullDependsTree.includes(versionModuleName)) {
      let message;
...
async function processCdnFonts(newText, fontsArray) {
   ...
   // for cdn fonts in files for IE application do replace url
...
function normalizeLink(extraData, partUrl, partFilePath, partExt, currentVersionHeader, ignoreQueryParams) {
   let result = `${partUrl}${partFilePath}${partExt}`;

   if (extraData) {
      result += `?${currentVersionHeader}`;

      // генератор шрифтов добавляет в css к урлам шрифтов ненужные таймштампы
      // при формировании нашей собственной ссылки на сервис статики они нам не
      // нужны и их нужно отбросить.
...
async function versionizeStyles(file, moduleInfo, options) {
   const {
      skipLogs,
      ignoreQueryParams,
      fontRoot,
   ...
   let newText = content.replace(
      ...
      (match, partUrl, partFilePath, partExt, extraData) => {
         // если нашли в ссылке плейсхолдер региона, необходимо сделать следующее:
         // 1) если в сборке 1 регион, нам нужно подменить плейсхолдер на переданный регион.
         // 2) если сборка мультирегиональная, нужно пометить файл как регионализированный и
         // записать для jinnee информацию о замене плейсхолдера региона в данном файле, чтобы
         // при сборке дистрибутивов он сделал все необходимые замены в указанных билдером файлах
         if (partFilePath.includes('%{REGION}')) {
            if (regions.length === 1) {
               // eslint-disable-next-line no-param-reassign
               match = match.replace('%{REGION}', regions[0]);
               // eslint-disable-next-line no-param-reassign
               partFilePath = partFilePath.replace('%{REGION}', regions[0]);
            } else {
               file.regionalized = true;
            }
         }
         ...
         if (partFilePath.indexOf('cdn/') > -1) {
            ...
            if (cdnUrl !== '/cdn/') {
               // если cdn-ресурсы просятся с СП, нам необходимо добавить заголовки версионирования к запросу
               let currentHeader;

               if (moduleInfo.joinedMeta) {
                  currentHeader = versionHeader.replace(
                     '%{MODULE_VERSION_STUB=%{modulePlaceholder}}',
                     moduleInfo.version
                  );
               } else {
                  currentHeader = versionHeader.replace(
                     '%{MODULE_VERSION_STUB=%{modulePlaceholder}}',
                     '%{BUILDER_VERSION_STUB}'
                  );
               }
         ...
         if (partFilePath.indexOf('%{CDN_ROOT}') > -1) {
            ...
            if (cdnUrl !== '/cdn/') {
               return normalizeLink(
                  extraData,
                  partUrl,
                  partFilePath.replace('%{CDN_ROOT}', cdnUrl),
                  partExt,
                  currentHeader,
                  ignoreQueryParams
               );
            ...
            return match.replace('%{CDN_ROOT}', cdnUrl);
         }
         ...
         if (moduleInfo.joinedMeta) {
            currentVersionHeader = versionHeader.replace('%{MODULE_VERSION_STUB=%{modulePlaceholder}}', moduleInfo.version);
         } else {
            currentVersionHeader = versionHeader.replace('%{modulePlaceholder}', versionModuleName);
         }

         // for fonts in files for IE application do replace url
         // with converted base64 format of font
         // if fontRoot is configured, try to load and convert it to base64
         if (
            ['.woff', '.woff2', '.eot', '.ttf'].includes(partExt) &&
            fontRoot
         ) {
            // fontRoot указывает, где взять
            const filePath = path.resolve(path.join(fontRoot, file.relative), '..', `${partFilePath}${partExt}`);
...
function versionizeTemplates(file, moduleInfo, options = {}) {
   const { skipLogs, cdnUrl, regions } = options;
   const content = file.contents.toString();
   const currentModuleName = path.basename(moduleInfo.output);
   let errors = false;
   const externalDependencies = new Set([]);
