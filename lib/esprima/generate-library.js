   ...
   traverse(ast, {
      enter(node) {
         if (node.type === Syntax.CallExpression && node.callee.type === Syntax.Identifier && node.callee.name === 'define') {
            ...
            privateDependenciesOrder.forEach((dep) => {
               ...
               try {
                  functionCallbackBody.body.splice(
                     packIndex,
                     0,
                     parse(`exports['${dep.moduleName}'] = true;`)
                  );

                  packIndex++;
               } catch (error) {
                  throw new Error(
                     `Esprima error: cant parse Javascript code: exports['${dep.moduleName}'] = true;\n` +
               ...
               try {
                  ...
                  throw new Error(
                     `Esprima error: cant parse generated code for private library part: ${moduleCode}\n` +
                     `Error message: ${error.message} \n` +
                     `Stack: ${error.stack}`
                  );
               }
                  ...
                  if (
                     ...
                     if (nativeJavascriptNames.includes(libraryParametersNames[privateDependencyIndex].name)) {
                        ...
                        try {
                           ...
                           throw new Error(
                              `Esprima error: cant parse generated code for alias: ${aliasCode}\n` +
                              `Error message: ${error.message} \n` +
                              `Stack: ${error.stack}`
                           );
                        }
            ...
            functionCallbackBody.body.push(parse(currentReturnExpressions.join('\n')));
