...
/**
 * Список селекторов, которые необходимо игнорировать.
 * Например, плагины borderSpacing, translate добавляют нежелательные конструкции
 * в результирующий файл, которые необходимо игнорировать.
 * @type {string[]}
 */
const RULES_TO_SKIP = [
   '*,::before,::after',
...
/**
 * Обойти каждое правило в css снимке.
 * @param {Object} snapshot CSS снимок.
 * @param {string[]} stack Стек имен.
 * @param {Function} callback Функция обработчик правил css.
 */
function forEachRule(snapshot, stack, callback) {
   for (const ruleName of Object.keys(snapshot)) {
      if (snapshot.hasOwnProperty(ruleName)) {
         const ruleValue = snapshot[ruleName];

         if (typeof ruleValue === 'string') {
            callback([...stack, ruleName], ruleValue, snapshot);

            continue;
         }

         forEachRule(ruleValue, [...stack, ruleName], callback);
...
class TailwindTreeShaker {
      ...
      walk(ast, {
         enter(node) {
            if (isLeaf(node)) {
               ...
               if (RULES_TO_SKIP.includes(rule)) {
                  return this.skip;
               }
   ...
   /**
    * Объединить текущее представление с css снимком.
    * @param {object} snapshot Css снимок.
    */
   merge(snapshot) {
      this.root = uniteSnapshots(snapshot, this.root);

      this.text = TailwindTreeShaker.generateText(this.root, this.prettyOutput);
    ...
    * @param {Object} snapshot CSS снимок.
    * @param {Function} callback Функция обработчик правил css.
    * Функция принимает аргументы ruleName типа string[], value типа string, owner типа Object.
    */
   static forEachRule(snapshot, callback) {
      forEachRule(snapshot, [], callback);
   }
    ...
    * @param {Object} snapshot CSS снимок.
    * @param {boolean} prettyOutput Форматировать текст отступами.
    * @returns {string} CSS текст.
    */
   static generateText(snapshot, prettyOutput) {
      const textBuilder = new TextBuilder(prettyOutput);

      createTextFromSnapshot(snapshot, textBuilder);
