...
function getTsConfigPath(tsconfig, configFile, nodeModulesPath, useStrictConfig) {
   if (tsconfig) {
   ...
   if (nodeModulesPath && fs.pathExistsSync(path.join(nodeModulesPath, 'saby-typescript'))) {
      sabyTypescriptDirectory = path.join(nodeModulesPath, 'saby-typescript');
   } else if (fs.pathExistsSync(path.join(cwd(), '../saby-typescript'))) {
   ...
   if (useStrictConfig) {
      defaultTsConfig = path.join(sabyTypescriptDirectory, 'configs', 'es5-strict.json');
   } else {
