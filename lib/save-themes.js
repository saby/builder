...
async function loadPlaceholderMeta(root, metaName, builderTests) {
   ...
   if (await fs.pathExists(filePath)) {
      ...
      await pMap(
         cachedVersionedMeta,
         async(currentFile) => {
            if (builderTests || (await fs.pathExists(path.join(root, currentFile)))) {
...
async function generateJoinedThemes(
   ...
   const placeholderMeta = await loadPlaceholderMeta(rootPath, 'versioned_modules.json', builderTests);
   const extraSuffixes = [''];
