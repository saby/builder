...
const { descendingSort, getFileHash } = require('./helpers');
...
async function generateThemeFile(
   ...
   // add each theme content hash into moduleHash calculation
   if (themesModuleInfo) {
      themesModuleInfo.addFileHash(themeFilePath, getFileHash(resultThemeContent, true));
   }
...
async function saveVersionedModules(root, array) {
   const filePath = getVersionedModulesPath(root);

   await fs.outputJson(filePath, array.sort());
}

async function saveLinkDependencies(root, linkDeps) {
   const filePath = path.join(root, 'ThemesModule/.builder/link_dependencies.json');

   await fs.outputJson(filePath, linkDeps);
}
...
async function generateJoinedThemes(
   ...
   await saveVersionedModules(rootPath, versionedModules);
   await saveLinkDependencies(rootPath, [...externalDependencies].sort());
}
