...
const getPlaceholderMetaPath = (rootPath, metaName) => path.join(rootPath, 'ThemesModule/.builder', metaName);
...
async function generateThemeFile(
   ...
   if (themesModuleInfo) {
      taskParameters.addFilesToCopy(themesModuleInfo.outputName, [themeFilePath]);
   }

   if (!placeholderMeta.includes(themeFilePath)) {
      placeholderMeta.push(themeFilePath);
   }
}

async function loadPlaceholderMeta(root, metaName) {
   const filePath = getPlaceholderMetaPath(root, metaName);
...
async function savePlaceHolderMeta(moduleRoot, metaName, appRoot, array) {
   const promises = [];

   if (moduleRoot !== appRoot) {
      promises.push(
         fs.outputJson(
            getPlaceholderMetaPath(moduleRoot, metaName),
            array.sort()
         )
      );
   }
   ...
   promises.push(
      fs.outputJson(
         getPlaceholderMetaPath(appRoot, metaName),
         array.sort()
      )
   );
...
async function generateJoinedThemes(
   ...
   const placeholderMeta = await loadPlaceholderMeta(rootPath, 'versioned_modules.json');
   const extraSuffixes = [''];
   ...
   await savePlaceHolderMeta(taskParameters.config.outputPath, 'versioned_modules.json', rootPath, placeholderMeta);
   await savePlaceHolderMeta(taskParameters.config.outputPath, 'cdn_modules.json', rootPath, placeholderMeta);
