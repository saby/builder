...
class DictionaryIndexer {
   constructor(localizations, countries, options = {}) {
      ...
      if (this.localizations.includes(locale)) {
         ...
         if (region) {
            ...
            if (toSafePosix(filePath) === toSafePosix(expectedFilePath)) {
               if (this.countries) {
                  // региональная сборка задана конфигурационным файлом
                  if (region.includes('-')) {
                     this.addDictionary(region.split('-')[0], locale);
                  } else {
   ...
   addLocalizationCSS(filePath, locale, text) {
      if (this.localizations.includes(locale)) {
         ...
         if (filePath.endsWith('.css')) {
            ...
            if (
               !this.cssStore[locale].some(
                  currentMeta => currentMeta.filePath === filePath
               )
            ) {
...
module.exports = {
   setDictionaryIndexer(moduleName, localizations, countries, meta) {
      indexer[moduleName] = new DictionaryIndexer(localizations, countries, meta);
   },
   indexer(moduleName) {
