...
const processingTmpl = require('../templates/processing-tmpl');
...
let globalComponentsProperties = [], parser;
...
function findWordsInDOM(words, text) {
   // ParserUtilities should be required only after collectWords function
...
async function findWordsInTMPL(words, text, filePath, componentsProperties) {
   ...
   if (traversedObj && traversedObj.hasOwnProperty('words')) {
      // add localization only for words that was translated manually in templates.
      // Do it to avoid false text nodes collecting in localization dictionary
      const result = traversedObj.words.filter(word => word.type === 'manual');
...
async function collectWords(modulePath, filePath, text, componentsProperties) {
   ...
   const prettyRelativePath = toSafePosix(
      path.join(
         moduleName,
   ...
   for (const word of words) {
      word.ui = moduleName;
      word.module = prettyRelativePath;
   }
   return words;
}
