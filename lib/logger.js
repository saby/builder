...
function registerFailedModule(moduleInfo) {
   if (!moduleInfo) {
      return;
   }

   const buildStatusStorage = getBuildStatusStorage();
   buildStatusStorage.registerFiledModule(moduleInfo);
}
...
class GulpLogger extends Logger {
   ...
   error(message) {
      ...
      registerFailedModule(message.moduleInfo);
   }
   ...
   */
   ...
   _addMessageForReport(level, obj, appInfo) {
      const result = {
         level,
         message: '',
         'cloud': appInfo.application,
         'responsibleOfCloud': appInfo.responsible
      };
      ...
      if (typeof obj === 'string') {
         ...
         if (obj.hasOwnProperty('moduleInfo') && obj.moduleInfo) {
            ...
            if (obj.moduleInfo.kaizen) {
               result.kaizenZone = obj.moduleInfo.kaizen.id;
               result.kaizenZoneUuid = obj.moduleInfo.kaizen.uuid;

               if (!result.responsibleOfModule) {
                  result.responsibleOfModule = obj.moduleInfo.kaizen.responsible;
               }
            }
         }
         if (obj.hasOwnProperty('filePath') && obj.filePath) {
   ...
   processWorkerCriticalErrors(logsPath) {
      const workerFolder = path.join(logsPath, 'worker');

      if (fs.existsSync(workerFolder)) {
         const filesToRead = fs.readdirSync(workerFolder);

         filesToRead.forEach((currentFileName) => {
            const errorMessage = fs.readJsonSync(path.join(workerFolder, currentFileName));

            this.error(errorMessage);
         });

         fs.rmSync(workerFolder, {
            recursive: true,
            force: true
         });
      }
   ...
   saveLoggerReport(logFolder) {
      ...
      this.processWorkerCriticalErrors(resultPath);
      const logsLevels = this.getMessagesLevel();
      const messages = this.getMessageForReport();
      let reportName = 'builder_report';
      if (process.argv.includes('runTypescript')) {
         reportName += '_tsc';
      }
      const reportFilePath = path.join(resultPath, `${reportName}.json`);
