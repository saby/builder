...
class TraversedCache {
    ...
    */
   constructor(allowIgnored = true) {
      /**
       * Разрешить использование ignored функционала.
       * @type {boolean}
       */
      this.allowIgnored = allowIgnored;
      ...
      /**
       * Коллекция игнорируемых узлов.
       * Используется при анализе циклических зависимостей: все узлы циклического пути заносятся сюда,
       * и далее игнорируются при анализе.
       * @type {Set<number>}
       */
   ...
   /**
    * Добавить узел в коллекцию игнорируемых.
    * @param {number} vertex Идентификатор вершины графа.
    */
   ignore(vertex) {
      this.ignored.add(vertex);
   }

   /**
    * Проверить, были ли вершина внесена в коллекцию игнорируемых.
    * @param {number} vertex Идентификатор вершины графа.
    * @returns {boolean} Возвращает true, если вершина внесена в коллекцию игнорируемых.
    */
   isIgnored(vertex) {
      return (
         this.allowIgnored && this.ignored.has(vertex)
      );
   }
...
class Digraph {
   ...
   /**
    * Добавить зависимость в граф, только если она не создает цикл.
    * @param {string} vertex Обрабатываемый узел.
    * @param {string} child Добавляемая зависимость.
    * @param {Function} onCycleHandler Обработчик циклической зависимости.
    */
   insertVertexChildSafe(vertex, child, onCycleHandler) {
      const children = this.get(vertex);

      try {
         this.modify(vertex, [...children, child]);

         this.testVertexCycles(vertex, (cyclePath) => {
            // Граф может содержать несколько циклических зависимостей.
            // Необходимо убедиться, что ребро (vertex; child) порождает цикл.
            for (let i = 0; i < cyclePath.length - 1; i++) {
               // vertex -> child
               if (cyclePath[i] === vertex && cyclePath[i + 1] === child) {
                  const cyclePathStr = [...cyclePath, cyclePath[0]].join(' -> ');

                  throw Object.assign(
                     new Error(`Cannot insert vertex child due to cycle: ${cyclePathStr}`),
                     {
                        vertex,
                        cyclePath,
                        cyclePathStr
         ...
         this.modify(vertex, children);

         onCycleHandler(error);
      }
   }
    ...
    * Метод позволяет получить минимально необходимый набор циклических зависимостей.
    * Полный набор при определенном объеме данных не всегда можно получить за отведенное время.
   ...
   /**
    * Проверяет наличие циклических зависимостей для конкретного узла.
    * @param {string} vertex Узел графа.
    * @param {Function?} onCycle Обработчик циклических зависимостей.
    */
   testVertexCycles(vertex, onCycle) {
      const cache = new TraversedCache(false);
      const vertexId = this._mapper.encode(vertex);

      if (typeof onCycle === 'function') {
         this._traverse(vertexId, [], cache, undefined, (cyclePath) => {
            onCycle(this._translateVertexes(cyclePath));
         });
   ...
   _onCycleErrorHandler(vertex, stack, cache, onCycle) {
      ...
      if (cyclePath.some(cache.isIgnored.bind(cache))) {
         // Уже обрабатывали циклический путь, в который входят узлы данного пути.
         // С большей долей вероятности ранее выведенной информации о циклическом пути
         // будет достаточно для исправления ошибки.
         return;
      }

      // Больше не выводим похожие циклические пути, которые состоят из данных вершин графа.
      stack.forEach(v => cache.ignore(v));
