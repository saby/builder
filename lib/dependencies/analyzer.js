...
/**
 * Коллекция UI модулей, которые автоматически указываются в less как зависимости.
 * @see lib/less/helpers.js
 * @type {Set<string>}
 */
const LessMixins = new Set([
   'SBIS3.CONTROLS',
...
/**
 * Получить коллекцию имен интерфейсных модулей, которые случайно оказались зацепленными в проекте.
 * Случайно зацепленным модулем считается модуль, присутствующий в проекте, но на который никто не ссылается
 * через зависимости, описанные в s3mod файле.
 * @param {Map<string, { name: string, depends: Set<string>, ref: ModuleInfo }>} modulesMap Список модулей.
 * @returns {Set<string>}
 */
function findHookedModules(modulesMap) {
   const hookedModules = new Set();

   const modulePostfixToSkip = [
      'CDN',
      'Font',
      'Icons',
      'Font-IE',
   ...
   modulesMap.forEach(({ name, ref }) => {
      if (modulePostfixToSkip.some(str => name.endsWith(str)) || moduleToIgnore.has(name)) {
         // Не рассматриваем определенные модули
         return;
      }

      if (Array.isArray(ref.featuresProvided) && ref.featuresProvided.length > 0) {
         // Не рассматриваем модули с фичами. Их зависимости задаются через фичи
         return;
      }

      if (Array.isArray(ref.featuresRequired) && ref.featuresRequired.length > 0) {
         // Не рассматриваем модули с фичами. Их зависимости задаются через фичи
         return;
      }
   ...
   const deleter = hookedModules.delete.bind(hookedModules);

   modulesMap.forEach((moduleInfo) => {
      moduleInfo.ref.depends.forEach(deleter);
   });
 ...
 * @returns {string} Возвращает инстанс ModuleInfo, если таковой был найден.
 */
 ...
 * @param {Map} files Коллекция исходных файлов.
 * @param {RequireJSModule} module Обрабатываемый модуль.
 * @param {string[]} depFiles Коллекция зависимостей.
 * @return {string[]} Возвращает коллекцию отфильтрованных файлов.
 */
function filterSources(files, module, depFiles) {
   const sourceFiles = depFiles
      .map(files.get.bind(files))

      // Исключаем third-party файлы
      .filter(sourceFile => !sourceFile.includes('/third-party/'));

   if (module.plugins.size > 0) {
      // TypeScript не проверяет существование зависимостей,
      // которые заданы с помощью плагинов.
      return sourceFiles;
   }
...
class Analyzer {
   ...
   async testLostDependencies(baseDir) {
      ...
      for await (const [rawModule, sources] of lost) {
         ...
         const filteredSources = filterSources(this.files, module, sources);
    ...
    * @param {boolean} checkHookedModules Учитывать в проверке случайно зацепленные модули.
    * Случайно зацепленным модулем считается модуль, присутствующий в проекте, но на который никто не ссылается
    * через зависимости, описанные в s3mod файле.
    * @returns {Object}
    */
   testUndeclaredUiDependencies(checkHookedModules) {
      const graph = createUIModulesGraph(this.modulesMap);
      const hookedModules = findHookedModules(this.modulesMap);

      this.stream.write(`[DEBUG] Получен набор hooked modules ${JSON.stringify(Array.from(hookedModules.values()), null, 3)}\n`);
      ...
      this.modulesMap.forEach((moduleInfo) => {
         ...
         if (undeclared.size > 0) {
            const undeclaredArray = Array.from(undeclared);
            const preparedSources = [];
            const preparedHookedSources = [];
            const artifactSources = [];

            for (const child of undeclaredArray) {
               let isSafeToInsert = true;
               let str = `\t* ${child}`;

               const childModuleId = this.modulesMap.has(child) ? this.modulesMap.get(child).ref.id : undefined;
               ...
               try {
                  graph.insertVertexChildSafe(moduleInfo.name, child, (error) => {
                     if (error.hasOwnProperty('cyclePath')) {
                        str += ' Добавление модуля приведет к циклической зависимости!';
                        isSafeToInsert = false;

                        this.stream.write(`[DEBUG] Добавление зависимости (${moduleInfo.name} -> ${child}) приводит к циклу: ${error.cyclePathStr}\n`);

                        return;
                     }

                     this.stream.write(`[DEBUG] Ошибка при выполнении insertVertexChildSafe: ${error.message}\n${error.stack}\n`);
                  });
               } catch (error) {
                  throw error;
               }
               ...
               if (isSafeToInsert) {
                  ...
                  // В модулях уже существуют циклические зависимости.
                  // Такие ошибки, к сожалению, выводить не можем.
                  if (hookedModules.has(child)) {
                     preparedHookedSources.push(str);
                  }
               }

               preparedSources.push(str);
            }

            if (artifactSources.length > 0) {
               artifact.push({
                  name: moduleInfo.name,
                  id: this.modulesMap.has(moduleInfo.name) ? this.modulesMap.get(moduleInfo.name).ref.id : undefined,
                  depends: artifactSources
               });
            }

            const sourcesStr = preparedSources.join('\n');

            this.diagnosticMessages.push({
               kind: 'warning',
               message: `В описании интерфейсного модуля ${moduleInfo.name}.s3mod не хватает зависимостей на другие модули:\n${sourcesStr}`,
               module: moduleInfo.ref.name
            });

            if (!checkHookedModules || preparedHookedSources.length === 0) {
               return;
            }

            const hookedSourcesStr = preparedHookedSources.join('\n');
            this.diagnosticMessages.push({
               kind: 'error',
               message: `В описании интерфейсного модуля ${moduleInfo.name}.s3mod не хватает зависимостей на другие модули:\n${hookedSourcesStr}`,
               module: moduleInfo.ref.name
      ...
      // Самопроверка. Полученный граф не должен содержать циклических зависимостей!

      graph.testCycles((cyclePath) => {
         const prettyCyclePath = [...cyclePath, cyclePath[0]];

         this.stream.write(`[ERROR] В дополненном графе обнаружена циклическая зависимость между интерфейсными модулями:\n${prettyCyclePath.map(v => `\t-> ${v}`).join('\n')}\n`);
      });
   ...
   testUiCycles() {
      ...
      graph.testCycles((cyclePath) => {
         ...
         this.diagnosticMessages.push({
            kind: 'error',
            message: `В проекте обнаружена циклическая зависимость между интерфейсными модулями: ${prettyCyclePath.join(' -> ')}`,
            module: this.modulesMap.get(cyclePath[0]).name
         });
      });
   }
         ...
         if (json.paths.hasOwnProperty(filePath)) {
            ...
            if (filePath.endsWith('.json')) {
               ...
               try {
                  ...
                  this.stream.write(`[DEBUG] Ошибка при обработке inputFiles.\n\tФайл: ${filePath}\n\tОшибка: ${error.message}\n`);
               }
         ...
         if (json.hasOwnProperty(filePath)) {
            if (filePath.endsWith('.less')) {
               json[filePath].forEach((name) => {
                  const uiName = transliterate(name.split('/').shift());

                  if (!LessMixins.has(uiName)) {
                     // Не добавляем зависимости, которые мы сами добавляем в less
                     lessDepends.add(uiName);
                  }
               });
            }
         }
   ...
   _putModule(filePath, moduleName, dependencies) {
      ...
      for (const dependency of (dependencies || [])) {
         ...
         if (target.hasPlugin('optional')) {
            // Не учитываем optional зависимости, какими бы они ни были, при анализе s3mod зависимостей
            continue;
         }
