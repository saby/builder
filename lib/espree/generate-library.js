...
function logDebugInfoIfJitsi(libraryName, message) {
      ...
      enter(node) {
         if (node.type === Syntax.CallExpression && node.callee.type === Syntax.Identifier && node.callee.name === 'define') {
            ...
            privateDependenciesOrder.forEach((dep) => {
               logDebugInfoIfJitsi(
                  libraryName,
                  `начали обработку зависимости ${dep.moduleName} библиотеки ${libraryName}`
               );
               ...
               const argumentsForClosure = dep.dependencies
                  .map((dependency) => {
                     ...
                     if (isPrivate(dependency) && !dependency.startsWith('i18n!')) {
               ...
               logDebugInfoIfJitsi(
                  libraryName,
                  `закончили обработку зависимости ${dep.moduleName}`
               );
            });
