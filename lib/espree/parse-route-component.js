...
function getReturnValuesOfFunction(body) {
   // For case of arrow function expression:
   // module.exports = () => ({ properties });
   if (body.right.type === Syntax.ArrowFunctionExpression) {
      if (body.right.body.type === Syntax.ObjectExpression) {
         return [
            body.right.body.properties
         ];
      }
   }

   // For case of other functions returning routing object.
   ...
   traverse(body.right.body, {
      enter(node) {
         ...
         if (innerFunctionDeclaration !== 0 || innerFunctionExpression !== 0) {
            return;
         }
...
function parseAssignment(assignmentNode, routes) {
   ...
   const isFunctionExpression = (
      assignmentNode.right.type === Syntax.FunctionExpression ||
      assignmentNode.right.type === Syntax.ArrowFunctionExpression
   );
