...
const { rebaseUrls, rebaseCdnRootUrls } = require('../../packer/lib/css-helpers');
...
async function getCompiledLess(lessContent, filePath, pathsForImport, imports, postCssOptions) {
   ...
   // из-за использования плейсхолдера %{CDN_ROOT} less-компилятор воспринимает url как относительный
   // и при импорте less с данным путём less-компилятор резолвит его как относительный, поэтому нам
   // необходимо в результирующей css провести корректировку путей и поправить данный путь на абсолютный
   if (result.includes('%{CDN_ROOT}')) {
      result = rebaseCdnRootUrls(result, multiService);
   }
