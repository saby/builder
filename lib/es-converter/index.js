...
async function generateSourceMap(components, target, sourceMapPaths) {
   ...
   await smModifier.initialize(components.sourceMap, sourceMapPaths);
...
async function generate(components, target, sourceMapPaths) {
   ...
   const sourceMap = await generateSourceMap(components, target, sourceMapPaths);
...
async function convert(program, source, target, options) {
   ...
   try {
      ...
      for (const moduleName of targets) {
         ...
         const { code, sourceMap } = await generate(components, moduleName, options.sourceMapPaths);
