...
function prepareCleanFactory(factory, fullSource, dependencies) {
   ...
   // В случае UMD имеем зависимости с повторениями, восстанавливаем имена их переменных.
   //    inUse у всех зависимостей истинный.
   // В случае AMD имеем зависимости, которые могут не соответствовать переменным (быть больше или меньше).
   //    inUse может быть ложным у некоторых зависимостей.
...
function modify(program, source, meta, options) {
   ...
   const result = {
      ...
      leadingData,
      trailingData,
      inlineSourceMap: meta.inlineSourceMap,
      sourceMap: meta.sourceMap,
      factoryLocation: meta.factory && meta.factory.loc
   };
   ...
   if (meta.dependenciesCallback) {
      result.dependenciesCall = source.slice(
...
function prepare(program, source, meta) {
...
function removeInlineSourceMap(source, meta) {
   if (meta.inlineSourceMap) {
      const [start, end] = meta.inlineSourceMapRange;
...
function modifyInlineSourceMap(meta, options) {
   if (typeof options.sourceMapPaths === 'undefined' || typeof meta.sourceMap === 'undefined') {
      return;
   }

   meta.sourceMap.file = options.sourceMapPaths.fileName;
   meta.sourceMap.sourceRoot = options.sourceMapPaths.sourceRoot;
   meta.sourceMap.sources = [options.sourceMapPaths.sourceFile];
}

module.exports = (program, sourceWithMaps, meta, options) => {
   const source = removeInlineSourceMap(sourceWithMaps, meta);

   modifyInlineSourceMap(meta, options);

   if (options.isCompiledFromTsc) {
      return modify(program, source, meta, options);
   }

   return prepare(program, source, meta);
};
