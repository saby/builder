...
async function analyzeProjectDependencies(modules, externalModules, logFolder, outputPath, checkHookedModules) {
   let stream;
   ...
   try {
      ...
      const artifact = await withTimer(stream, 'testUndeclaredUiDependencies', () => analyzer.testUndeclaredUiDependencies(checkHookedModules));

      await withTimer(stream, 'writeArtifact', () => fs.writeJson(path.join(logFolder, 'undeclared-ui.json'), artifact, 'utf-8'));
