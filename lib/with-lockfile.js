   ...
   for (let i = 0; i < maxAttemptCount; ++i) {
      try {
         if (await lock(filePath)) {
            logger.debug(`Created lockfile "${filePath}"`);
            ...
            try {
               ...
               if (await unlock(filePath)) {
                  logger.debug(`Removed lockfile "${filePath}"`);
               } else {
         ...
         logger.debug(`Busy lockfile "${filePath}". Next attempt in ${timeout} ms.`);
...
module.exports.toFileName = (p, key) => path.join(p.config.cachePath, DIRECTORY_NAME, key);
