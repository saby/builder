...
function getChangedFilesList(taskParameters, moduleInfo, filterFunction, options = {}) {
   ...
   const failedFiles = moduleInfo.cache.getFailedFiles()
      .map((currentRelativePath) => {
         // если файл с предыдущей сборки был зафейлен, нам необходимо проставить признаки пересборки конкретных
         // функциональных областей(сборка typescript, сборка метатипов, локализации и тд).
         moduleInfo.checkForCommonExtensions(currentRelativePath, 'change');
         return path.join(moduleInfo.path, currentRelativePath);
      })
      .filter(filterFunction);
   ...
   if (result.length === 0) {
      normalizedResult = [path.join(moduleInfo.appRoot, moduleInfo.name)];
      taskParameters.addCurrentModuleInputs(moduleInfo.name, normalizedResult);
   ...
   normalizedResult = [...new Set(result)];
   taskParameters.addCurrentModuleInputs(moduleInfo.name, normalizedResult);
...
function getModuleInputForCompress(taskParameters, moduleInfo, moduleOutput) {
   ...
   if (!currentFiles) {
      ...
      const hasNoChanges = moduleHasNoChanges(
         moduleInfo,
         [
      ...
      // В модулях с иконками необходимо обрабатывать только корневые SVG файлы
   ...
   function filterFile(file) {
      // В модулях с иконками необходимо обрабатывать только корневые SVG файлы
      if (moduleInfo.name.endsWith('-icons')) {
         return anyMatch('*.svg', file) || file.match(MINIFIED_FILE_TO_COMPRESS);
      }

      return file.endsWith('.svg') || file.match(MINIFIED_FILE_TO_COMPRESS);
   }

   const filesToRead = Array.from(currentFiles).filter(filterFile);
