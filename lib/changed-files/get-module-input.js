...
function getInputForDropCacheCases(taskParameters, moduleInfo) {
   ...
   // TODO change html minifier to https://www.npmjs.com/package/html-minifier-terser
   //  because our old minificator do partial minification(after one more minification it can
   //  delete additional whitespaces).
   //  https://online.sbis.ru/opendoc.html?guid=176e5709-9878-4bdc-af0f-81b4e9937fe8&client=3
   if (taskParameters.config.deprecatedStaticHtml) {
      result.push(path.join(moduleInfo.path, '/**/*.html'));
   }
...
function getChangedFilesList(taskParameters, moduleInfo, filterFunction, options = {}) {
   const { additionalPatterns, dropCachePatterns } = options;
   const result = [];
   ...
   result.push(
      ...taskParameters.cache.getFilesWithErrors(moduleInfo.path)
         .filter(filterFunction)
   );
...
function getModuleInputForBuild(taskParameters, moduleInfo, gulpSrcOptions) {
   ...
   if (moduleInfo.changedFiles && !taskParameters.cache.hasIncompatibleChanges && !moduleInfo.outputIsClean) {
      const options = {
...
function getModuleInputForCompile(taskParameters, moduleInfo, gulpSrcOptions) {
   ...
   if (moduleInfo.changedFiles && !taskParameters.cache.hasIncompatibleChanges && !moduleInfo.outputIsClean) {
      ...
      const result = getChangedFilesList(taskParameters, moduleInfo, isSourceFile);
