...
const { supportedBrowsers, supportedBrowsersForIEApp } = require('./builder-constants');
...
function runMinifyCss(options) {
   ...
   if (newMinimizer) {
      let targets;
      if (buildIE) {
         targets = browserslistToTargets(browserslist(supportedBrowsersForIEApp));
      } else {
         targets = browserslistToTargets(browserslist(supportedBrowsers));
      }
      try {
         compiled = transform({ ...LIGHTNINGCSS_OPTIONS, targets, code: Buffer.from(text) });
      } catch (err) {
