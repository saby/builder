...
function getHalfMemoryLimit() {
   return Math.trunc(TOTAL_MEMORY * 0.5);
}

function getHeapSizeCommand() {
   if (isWindows) {
      return `set NODE_OPTIONS="--max-old-space-size=${getHalfMemoryLimit()}"`;
   }
   return `export NODE_OPTIONS='--max-old-space-size=${getHalfMemoryLimit()}'`;
}
...
function useCompiledFiles(taskParameters, moduleInfo) {
   return taskParameters.config.compiled &&
      taskParameters.cache.isFirstBuild() &&
      typeof moduleInfo.compiled === 'boolean' &&
      moduleInfo.compiled;
}

// функция, которая вычисляет, есть ли в модуле изменения по различным переданным признакам
// Вернёт true только при выполнении следующих условий:
// 1) если модуль работает в режиме сборки по изменениям.
// 2) если по переданным признакам изменений не обнаружено -
// в список передаются признаки наличия изменений по каждой соответствующей задаче
// (например для сборки contents есть изменения локализации или есть изменения в tailwind.css для данного модуля)
// 3) не было глобального сброса кеша билдера
// 4) не было сброса кеша для конкретного интерфейсного модуля
function moduleHasNoChanges(moduleInfo, specificConditions) {
   return moduleInfo.changedFiles &&
      !specificConditions.some(currentCondition => !!currentCondition) &&
      !getBuildStatusStorage().cacheIsDropped &&
      !moduleInfo.forceRebuild;
}

// в модуле WS.Core есть legacy-код, который написан без "use strict" и при портировании на ts компилятор
// ставит эту директиву по умолчанию, что ломает поведение legacy-кода. В этом случае мы удаляем
// use strict сами при наличии в исходнике нашей пользовательской директивы // @ts-use-strict-false
function removeStrictMode(contents) {
   const code = contents.toString();

   return Buffer.from(
      code
         .replace('// @ts-use-strict-false', '')
         .replace('"use strict";', '')
   );
}
