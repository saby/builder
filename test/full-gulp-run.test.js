...
describe('builder', () => {
   it('full gulp build must be completed successfully', async() => {
      ...
      const config = {
         ...
         modules: [
            ...
            },
            {
               ...
               path: path.join(sourceFolder, 'ParametersWebAPI'),
               depends: [
                  'Env',
                  'Browser',
                  'Types'
            ...
            {
               name: 'Meta',
               path: path.join(sourceFolder, 'Meta'),
               depends: [
                  'Types',
                  'Env',
                  'Application',
      ...
      try {
         ...
         // валим тест если были ошибки уровня ERROR. Падать могут также
         // ворнинги с кодом 6, причем по вине модулей платформы,
         // которые мы собираем для тестов, в таком случае пропускаем.
         if (err.code === 1) {
            throw err;
         }
      }
