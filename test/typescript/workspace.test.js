...
function createTaskParameters(cwd) {
   ...
   return new TaskParameters(config);
}
...
describe('gulp/builder/generate-task/typescript/workspace', () => {
   ...
   it('should prepare tsc cache directory', async() => {
      ...
      expect(taskParameters.typescriptOutputDir).equals(
         '/workspace/cache/typescript-cache/a13411350d6d5c13c8f43b13797373fd6d36df8b/emit'
      );
   });
   ...
   it('should drop tsc cache 1', async() => {
      ...
      expect(
         fakeFs.overrides.promises.rm.calledWith(taskParameters.typescriptOutputDir)
      ).to.be.true;
   });
