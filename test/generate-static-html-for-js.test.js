...
const config = {
   root: toSafePosix(path.join(dirname, 'fixture/generate-static-html-for-js')),
   application: '/',
   userParams: true,
   globalParams: false,
   urlServicePath: '/',
...
const modules = new Map([
   ['Модуль', toSafePosix(path.join(dirname, 'fixture/generate-static-html-for-js/Modules/Модуль'))],
   [
      'Тема Скрепка',
      toSafePosix(path.join(dirname, 'fixture/generate-static-html-for-js/Modules/Тема Скрепка'))
   ],
   ['Ошибки', toSafePosix(path.join(dirname, 'fixture/generate-static-html-for-js/Modules/Ошибки'))]
]);
   ...
   describe('module with web page', () => {
      it('empty', async() => {
         ...
         const moduleInfo = {
            ...
            fullDependsTree: ['Тема Скрепка', 'Модуль']
         };
      ...
      it('recursive includes error', () => {
         ...
         const moduleInfo = {
            ...
            fullDependsTree: ['Тема Скрепка', 'Модуль', 'Ошибки']
         };
         const root = toSafePosix(path.join(dirname, 'fixture/generate-static-html-for-js'));
         return generateStaticHtmlForJs(
            'virtualFile',
            componentInfo,
            moduleInfo,
