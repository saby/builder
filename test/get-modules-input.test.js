...
describe('getModuleInput', () => {
   ...
   describe('changedFiles is an empty array', () => {
      ...
      describe('check patterns for drop cache flags', () => {
         it('compile task - should be added "lang" namespace pattern to read if localization is enabled', async() => {
   ...
   describe('changedFiles meta has some files to build', () => {
      describe('compile task - full meta ts namespace adding', () => {
         ...
         it('should be ignored if no changed files in dependant interface modules', async() => {
            const gulpConfig = {
               cache: './cache',
               modules: [{
                  name: 'Module-meta',
                  path: './Module-meta',
                  changedFiles: [],
                  depends: ['Module1', 'Module_missing']
               }, {
                  name: 'Module1',
                  path: './Module1',
                  changedFiles: [],
            ...
            expect(result).to.have.members([`${sourceFolderPath}/Module-meta`]);
         });

         it('should be added if at least 1 dependant interface module has changed files', async() => {
            const gulpConfig = {
               cache: './cache',
               modules: [{
                  name: 'Module2',
            ...
            const { taskParameters, moduleInfo, gulpSrcOptions } = await generateFullEnvironment(gulpConfig, 2);
         ...
         it('should be added if at least 1 dependant interface module has deleted files', async() => {
            const gulpConfig = {
               cache: './cache',
               modules: [{
                  name: 'Module2',
         ...
         it('should be added if at least for 1 dependant interface module changedFiles is undefined', async() => {
            const gulpConfig = {
               cache: './cache',
               modules: [{
                  name: 'Module2',
