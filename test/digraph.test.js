...
describe('lib/struct/digraph', () => {
   ...
   it('should have cycles in graph', () => {
      ...
      graph.testCycles(cycles.push.bind(cycles));

      expect(cycles).deep.equal([
         [1, 17, 16, 12, 5]
      ]);
   });

   it('should have cycles in graph 2', () => {
      graph.modify(16, [11, 12, 13, 18]);
   ...
   it('should have cycles in graph 3', () => {
      graph.modify(6, [2, 3, 16]);

      const cycles = [];

      // Метод выведет не все возможные циклы, а только те, в которых узлы не повторяется
      // Этого необходимо и достаточно для исправления ошибки
   ...
   it('should put child to vertex', () => {
      let cyclePath;

      graph.modify(3, [2, 6]);

      graph.insertVertexChildSafe(6, 16, (error) => {
         ({ cyclePath } = error);
      });

      expect(cyclePath).deep.equal([
         6, 16, 11
      ]);
   });

   it('should put child to vertex 2', () => {
      let cyclePath;

      graph.insertVertexChildSafe(2, 1, (error) => {
         ({ cyclePath } = error);
      });

      expect(cyclePath).equals(undefined);
   });
});
