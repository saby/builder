...
it('should return correct compilerOptions in depends of content format(basic ts module or amd-formatted)', () => {
   ...
   // if ts module amd-formatted, compilerOptions should contain "module" option with value 'commonjs'
   result.compilerOptions.module.should.equal('commonjs');
   ...
   result.compilerOptions.module.should.equal('amd');
});
...
describe('tsconfig path', () => {
   ...
   it('should return default tsconfig path if non transmitted or transmitted is not found', async() => {
      let tsConfigPath = await getTsConfigPath('', testConfigPath, undefined, true);
      toPosix(tsConfigPath).endsWith('saby-typescript/configs/es5-strict.json').should.equal(true);

      tsConfigPath = await getTsConfigPath('', testConfigPath, undefined, false);
