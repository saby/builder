...
describe('parse js component', () => {
   ...
   it('empty file', () => {
      const result = parseJsComponent('', parseJsComponentOptions).componentInfo;
      Object.getOwnPropertyNames(result).length.should.equal(0);
   });
   it('file with error', () => {
      expect(() => {
         parseJsComponent('define(', parseJsComponentOptions);
      }).to.throw('Line 1: Unexpected end of input');
   });
   it('empty module name', () => {
      const result = parseJsComponent('define(function(){});', parseJsComponentOptions).componentInfo;
      Object.getOwnPropertyNames(result).length.should.equal(0);
   });
   it('normal module name', () => {
      const result = parseJsComponent('define("My.Module/Name", function(){});', parseJsComponentOptions).componentInfo;
   ...
   it('declare dependencies module', () => {
      const result = parseJsComponent('define("My.Module/Name", ["My.Dep/Name1", "My.Dep/Name2"], function(){});', parseJsComponentOptions).componentInfo;
   ...
   it('declare dependencies module, empty name', () => {
      const result = parseJsComponent('define(["My.Dep/Name1", "My.Dep/Name2"], function(){});', parseJsComponentOptions).componentInfo;
      Object.getOwnPropertyNames(result).length.should.equal(1);
      result.componentDep.should.have.members(['My.Dep/Name1', 'My.Dep/Name2']);
      result.hasOwnProperty('isNavigation').should.equal(false);
   });
   ...
   describe('typescript dynamic import checker', () => {
      ...
      const testCommonCase = async(fileName) => {
         ...
         const result = parseJsComponent(removeRSymbol(text.toString()), parseJsComponentOptions).componentInfo;
      ...
      describe('should be patched in case of using require and not having it\'s own catch errors callback', () => {
         it('promise as new expession', async() => {
            ...
            const result = parseJsComponent(removeRSymbol(text.toString(), parseJsComponentOptions), {
            ...
            result.amdContent.should.equal('define("TestModule/myModule", ["require", "exports"], function (require, exports) {\n' +
               ...
               '    }))\n' +
               '});');
            const minifiedResult = runUglifyJs('virtual.js', result.amdContent);
