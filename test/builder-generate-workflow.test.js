...
describe('gulp/builder/generate-workflow.js', () => {
   ...
   it('should recompile properly after source directory renamed without cache reset', async() => {
      ...
      const renamedLogFolder = path.join(`${workspaceFolder}-1`, 'logs');
      config = {
         cache: renamedCacheFolder,
         output: renamedOutputFolder,
         logs: renamedLogFolder,
         less: true,
         modules: [
            {
               name: 'Controls-default-theme',
   ...
   it('check-common-meta', async() => {
      ...
      const builderCacheMetaOutput = path.join(cacheFolder, 'incremental_build/Module/.builder');
      ...
      const testMeta = async(metaOutput) => {
         ...
         (await isRegularFile(metaOutput, 'moduleHash')).should.equal(true);
         ...
         const moduleHashList = await fs.readJson(path.join(metaOutput, 'moduleHash-list'));
         ...
         Object.keys(moduleHashList).should.have.members([
      ...
      // proper meta data should be in both cache and output versions of
      // builder meta files and must have same content between each other
      await testMeta(builderMetaOutput);
      await testMeta(builderCacheMetaOutput);
   ...
   describe('custom pack', () => {
      ...
      it('patch - after patch for module "Modul" other interface modules cache must not be affected(files removal)', async() => {
         ...
         testDirectory.should.have.members([
            ...
            'interfacemodule1-styles-in-js.package.min.js',
            'interfacemodule1-styles-in-js.package.min.js.lockfile',
            'interfacemodule1-styles-in-js-optional.package.min.js',
            'interfacemodule1-styles-in-js-optional.package.min.js.lockfile',
            'optionalBundles.json',
            'optionalBundles.min.js',
            'interfacemodule1.package.min.css',
            'interfacemodule1.package.min.js',
            'interfacemodule1.package.min.js.lockfile',
            'interfacemodule1.package_ie.min.css',
            'library.ts',
            'library.js',
            'library.js.map',
            'library.min.js',
   ...
   it('rtl css packages with compiled module', async() => {
      ...
      const testResults = async() => {
         ...
         await checkDirIsSymlink(path.join(cacheFolder, 'incremental_build/InterfaceModule1'));
   ...
   it('pack static html - check paths validity to static packages', async() => {
      ...
      const testMultiServiceResults = async() => {
         const correctPackagesList = [
            'TestModule/static_packages/b3ab555b092af336ef1432adc371f8d1.min.js',
