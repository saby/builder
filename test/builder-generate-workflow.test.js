...
describe('gulp/builder/generate-workflow.js', () => {
   ...
   it('check correct output of patches sequence', async() => {
      ...
      const checkChanges = async(changesContent, fileName, outputHasChanges) => {
         await pMap(
            ['.ts', '.js'],
            async(extension) => {
      ...
      config.modules[0].changedFiles = ['./Модуль.ts'];
      config.modules[0].deletedFiles = [];
      config.modules[1].rebuild = true;
      config.modules[1].changedFiles = [];
      config.modules[1].deletedFiles = [];
      config.changedFilesOutput = outputFolder;
      await fs.writeJSON(configPath, config);

      let content = await fs.readFile(path.join(sourceFolder, 'Modul/Модуль.ts'));
      await fs.outputFile(
         path.join(sourceFolder, 'Modul/Модуль.ts'),
      ...
      await checkChanges('/* patch 1 */', 'Modul/Modul', true);
      ...
      (await isRegularFile(path.join(outputFolder, 'Modul'), 'Modul.min.js.gz')).should.equal(true);
      (await isRegularFile(path.join(outputFolder, 'Modul'), 'Modul.min.js.br')).should.equal(true);
      await fs.remove(path.join(moduleOutputFolder, 'Modul.min.js.br'));
      await fs.remove(path.join(moduleOutputFolder, 'Modul.min.js.gz'));
      ...
      // Modul1 не должно быть изменений в конечной директории сборки, поскольку изменения из предыдущего
      // патча в данный патч попасть не должны
      await checkChanges('/* patch 2 */', 'Modul2/Module2', true);
      await checkChanges('/* patch 1 */', 'Modul/Modul', false);
      ...
      config.modules[1].changedFiles = ['./Module2.ts'];
      config.modules[1].deletedFiles = [];
      delete config.changedFilesOutput;
      await fs.writeJSON(configPath, config);
   ...
   it('compile themes', async() => {
      ...
      await prepareTest(fixtureFolder);
      const checkBasicResults = async(checkIEResults) => {
         ...
         // 4) compiled style has content from theme.json file
         defaultThemeContent.should.includes('/* Controls-default-theme/theme */\n' +
            ...
            '   --unaccented_text-color: #999;\n' +
            '   --icon: url(\'/resources/Controls-default-theme/img/test.svg?x_module=test\');\n' +
            '   --readonly_text-color: #ccc;\n' +
      ...
      // run flow
      await runWorkflowWithTimeout();
      await checkBasicResults(false);

      // run flow again to check incremental build works properly
      config.ESVersion = 5;
      await fs.writeJSON(configPath, config);
      await runWorkflowWithTimeout();
      await checkBasicResults(true);
   ...
   it('new type locales: must compile js-locale and write it to contents only for existing json-locales', async() => {
      ...
      const testResults = async() => {
         ...
         contents.should.deep.equal(correctCommonContents);
         const enContents = await fs.readJson(path.join(moduleOutputFolder, 'contents-en.json'));
         enContents.should.deep.equal(correctEnContents);
   ...
   it('compile less', async() => {
      ...
      let stableCss = await fs.readFile(path.join(moduleOutputFolder, 'Stable.css'), 'utf8');
      ...
      // чтобы проверить правильную простановку дефолтных значений для css variables,
      // нужно в конфиге сборки проставить ESVersion 5 и перезапустить сборку
      config.ESVersion = 5;
      await fs.writeJSON(configPath, config);
      await runWorkflowWithTimeout();

      stableCss = await fs.readFile(path.join(moduleOutputFolder, 'Stable.css'), 'utf8');
   ...
   it('check-common-meta', async() => {
      ...
      const testMeta = async(metaOutput) => {
         ...
         (await isRegularFile(moduleOutput, 'Module.types')).should.equal(true);
         ...
         const metaTypes = await fs.readJson(path.join(moduleOutput, 'Module.types'));
   ...
   describe('custom pack', () => {
      ...
      it('private packages should be generated despite of theirs not existing in bundles approved list', async() => {
         ...
         removeRSymbol(removeRSymbol(resultPackage)).should.equal(removeRSymbol(correctResultPackage));
      });
      ...
      it('custom package with lazy modules initialization should be generated correctly', async() => {
         ...
         removeRSymbol(removeRSymbol(privateLazyPackage)).should.equal(removeRSymbol(correctPrivateLazyPackage));
      });
      it('optional bundles must be written in a special meta and should be includes into common bundles meta', async() => {
         ...
         // check also localized contents to check info about bundles
         const currentEnContents = await fs.readJson(path.join(outputFolder, 'InterfaceModule1/contents-en.json'));
         currentEnContents.modules.InterfaceModule1.hasOptionalBundles.should.equal(true);
         currentEnContents.modules.InterfaceModule1.hasBundles.should.equal(true);
      });
      ...
      it('gzip and brotli - check for brotli correct encoding and decoding. Should compressed only minified and packed', async() => {
         ...
         const correctMembers = [
            ...
            'contents-en.json',
            'contents-en.json.js',
            'contents-en.json.min.js',
            'contents-en.json.min.js.br',
            'contents-en.json.min.js.gz',
            'contents-en.min.json',
            'contents-en.min.json.br',
            'contents-en.min.json.gz',
            'contents-ru.json',
            'contents-ru.json.js',
            'contents-ru.json.min.js',
            'contents-ru.json.min.js.br',
            'contents-ru.json.min.js.gz',
            'contents-ru.min.json',
            'contents-ru.min.json.br',
            'contents-ru.min.json.gz',
            'Stable.css',
            'Stable.less',
            'Stable.min.css',
            'Stable.min.css.gz',
      ...
      it('versioned modules meta should have correct members', async() => {
         const testResults = async() => {
            ...
            versionedModules.should.have.members([
               ...
               'Modul/contents-en.json',
               'Modul/contents-en.json.js',
               'Modul/contents-en.json.min.js',
               'Modul/contents-en.min.json',
               'Modul/contents-ru.json',
               'Modul/contents-ru.json.js',
               'Modul/contents-ru.json.min.js',
               'Modul/contents-ru.min.json'
            ]);
      ...
      it('rtl css packages', async() => {
         ...
         const testResults = async() => {
            ...
            const correctCssPackage = await fs.readFile(`${correctPackagePath}.css`, 'utf8');
            const correctJsPackage = await fs.readFile(`${correctPackagePath}.js`, 'utf8');
            removeRSymbol(removeRSymbol(compiledCssPackage)).should.equal(removeRSymbol(correctCssPackage));
            removeRSymbol(removeRSymbol(compiledJsPackage)).should.equal(removeRSymbol(correctJsPackage));
         };
      ...
      it('builder must build only changed files', async() => {
         ...
         const testResults = async() => {
            const changedFilesList = await fs.readJson(path.join(logsFolder, 'builder-metrics.json'));
            changedFilesList.changedModules.InterfaceModule1.outputArtifacts.should.have.members([
               'amdModule.min.css',
               'amdModule.css',
               'library.ts',
               'library.min.js',
         ...
         const currentConfig = {
            ...
            modules: [
               ...
               },
               {
                  name: 'InterfaceModule1',
                  path: path.join(sourceFolder, 'InterfaceModule1'),
                  changedFiles: ['amdModule.css', 'library.ts'],
                  deletedFiles: []
               }
            ]
         };
   ...
   it('pack inline scripts - check for current packing of inline scripts into separated javascript files', async() => {
      // оставим дебажные логи, чтобы отладить нестабильную ошибку на маке, которая стреляет раз в 2 месяца при сборке
      // и прогонах тестов билдера на MacOS, нужно понимание, что происходит в логах билдера в момент падения ошибки
      console.log('Начало работы теста pack inline scripts');
      const fixtureFolder = path.join(dirname, 'fixture/builder-generate-workflow/packHTML');
      await prepareTest(fixtureFolder);
      await linkPlatform(sourceFolder);
      ...
      await runTestIteration('%{RESOURCE_ROOT}');
      console.log('Конец работы теста pack inline scripts');
      await clearWorkspace();
      console.log('pack inline scripts - workspace почищен');
   });
   ...
   it('compile es and ts', async() => {
      const checkFiles = async() => {
         ...
         removeRSymbol(TsxDevContent.toString()).should.equal(
            'define("Modul/ReactTest", ["require", "exports", "react/jsx-dev-runtime"], function (require, exports, jsx_dev_runtime_1) {\n' +
            '    "use strict";\n' +
            '    Object.defineProperty(exports, "__esModule", { value: true });\n' +
            '    const _jsxFileName = "\\u041C\\u043E\\u0434\\u0443\\u043B\\u044C/ReactTest.tsx";\n' +
            '    function Square(props) {\n' +
            '        return ((0, jsx_dev_runtime_1.jsxDEV)("button", { className: "square", onClick: props.onClick, children: props.value }, void 0, false, { fileName: _jsxFileName, lineNumber: 2, columnNumber: 13 }, this));\n' +
            '    }\n' +
         ...
         removeRSymbol(TsxProdContent.toString()).should.equal(
            'define("Modul/ReactTest",["require","exports","react/jsx-runtime"],(function(e,t,u){' +
            '"use strict";Object.defineProperty(t,"__esModule",{value:true}),' +
            't.default=function(e){return(0,u.jsx)("button",{className:"square",onClick:e.onClick,children:e.value})}' +
            '}));'
         );
      };
   ...
   describe('check interfaces build and packing', () => {
      ...
      const checkResults = async(
         ...
         if (checkContents) {
            const contentsResult = await fs.readJson(`${outputFolder}/contents.json`);
            contentsResult.modules.ModuleWithAPI.features[currentInterface].should.be.equal(providerName);
         }
      };
      ...
      it('sources enabled, joinedMeta disabled, 3 providers - 1 in base interface and 2 in different modules', async() => {
      ...
      it('sources disabled, joinedMeta disabled, 3 providers - 1 in base interface and 2 in different modules', async() => {
      ...
      it('sources enabled, joinedMeta enabled, 3 providers - 1 in base interface and 2 in different modules', async() => {
         const config = { ...defaultConfig };
      ...
      it('sources enabled, joinedMeta enabled, 2 providers - 1 in base interface and 1 in another module', async() => {
         // remove second provider and check that base interface will be packed
         // with a first one
         const config = { ...defaultConfig };
         delete config.modules[2].featuresProvided;
         config.sources = true;
         config.joinedMeta = true;
      ...
      it('sources disabled, joinedMeta enabled, 2 providers - 1 in base interface and 1 in another module', async() => {
         const config = { ...defaultConfig };
         config.sources = false;
         config.joinedMeta = true;
         await fs.writeJSON(configPath, config);
      ...
      it('sources enabled, joinedMeta enabled, 1 provider from base interface itself', async() => {
         // remove providers from another modules and check that base interface will be packed
   ...
   describe('pack-library', () => {
      ...
      it('workspace-generated', async() => {
         ...
         await pMap(
            ...
            async(basename) => {
               const contents = await fs.readFile(path.join(correctModulesPath, basename), 'utf8');

               correctModulesContent[basename] = removeRSymbol(
                  contents.slice(contents.indexOf('define('), contents.length)
               );
            },
            {
               concurrency: 5
      ...
      it('test-recurse-library-dependencies-in-store', async() => {
         ...
         const dependenciesStore = await fs.readJson(path.join(outputFolder, 'Modul/.cache/dependencies.json'));
