...
describe('process svg', () => {
   ...
   it('check common plugin', async() => {
      ...
      const testResults = async(filteringPackageExists) => {
         ...
         (await isRegularFile(iconsOutputFolder, 'sorting.css')).should.equal(true);
         (await isRegularFile(iconsOutputFolder, 'sorting.eot')).should.equal(true);
         (await isRegularFile(iconsOutputFolder, 'sorting.html')).should.equal(true);
         (await isRegularFile(iconsOutputFolder, 'sorting.woff')).should.equal(true);
         (await isRegularFile(iconsOutputFolder, 'sorting.woff2')).should.equal(true);
