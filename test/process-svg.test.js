...
describe('process svg', () => {
   ...
   it('check common plugin', async() => {
      ...
      const testResults = async(filteringPackageExists) => {
         ...
         // but sorting namespace is valid for fonts generator, so check common font files in root
         (await isRegularFile(iconsOutputFolder, 'sorting_s.svg')).should.equal(true);
         (await isRegularFile(iconsOutputFolder, 'sorting.svg')).should.equal(false);
         (await isRegularFile(iconsOutputFolder, 'sorting_s.css')).should.equal(true);
         (await isRegularFile(iconsOutputFolder, 'sorting_s.eot')).should.equal(true);
         (await isRegularFile(iconsOutputFolder, 'sorting_s.html')).should.equal(true);
         (await isRegularFile(iconsOutputFolder, 'sorting_s.woff')).should.equal(true);
         (await isRegularFile(iconsOutputFolder, 'sorting_s.woff2')).should.equal(true);
