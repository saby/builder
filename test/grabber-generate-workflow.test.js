...
const checkResult = async function(extension, context) {
   ...
   if (['wml', 'tmpl'].includes(extension)) {
      resultObj[0].type.should.equals('manual');
   }
   resultObj[0].context.should.equals(context);
   resultObj[0].ui.should.equals('Modul');
   resultObj[0].module.should.equals(`Modul/Component.${extension}`);
};
...
describe('gulp/grabber/generate-workflow.js', () => {
   ...
   describe('check localization words collector for js', () => {
      it('rebuild without changes', async() => {
         const fixtureFolder = path.join(dirname, 'fixture/grabber-generate-workflow/javascript');
   ...
   describe('check localization words collector for ts', () => {
      it('rebuild without changes', async() => {
         const fixtureFolder = path.join(dirname, 'fixture/grabber-generate-workflow/typescript');
         await prepareTest(fixtureFolder);
         await fs.writeJSON(configPath, config);
   ...
   describe('check localization words collector for xhtml code', () => {
      it('rebuild without changes', async() => {
         const fixtureFolder = path.join(dirname, 'fixture/grabber-generate-workflow/xhtml');
         await prepareTest(fixtureFolder);
         await fs.writeJSON(configPath, config);
   ...
   describe('check localization words collector for tmpl code', () => {
      it('rebuild without changes', async() => {
         const fixtureFolder = path.join(dirname, 'fixture/grabber-generate-workflow/tmpl');
         await prepareTest(fixtureFolder);
         await fs.writeJSON(configPath, config);
      ...
      it('rebuild with changes in tmpl code', async() => {
         ...
         await fs.writeFile(testFilePath, testFileText.replace('{[AnyContext@@AnyText]}', 'AnyContext@@AnyText'));
   ...
   describe('check localization words collector for wml code', () => {
      it('rebuild without changes', async() => {
         const fixtureFolder = path.join(dirname, 'fixture/grabber-generate-workflow/wml');
         await prepareTest(fixtureFolder);
         await fs.writeJSON(configPath, config);
