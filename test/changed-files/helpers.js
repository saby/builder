...
const DEFAULT_CONFIG = {
   ...
   modules: [{
      name: 'Module1',
      path: './Module1',
      changedFiles: ['./test.ts', './test.less', './test.wml', './test.xhtml', './test.tmpl', './test.json']
   }]
};


function generateSimpleEnvironment(module) {
   const moduleInfo = new ModuleInfo(module, null, { output: '/path/to/output/' });
...
async function generateFullEnvironment(gulpConfig, moduleIndex = 0) {
   const config = new Configuration();
   ...
   const moduleInfo = config.modules[moduleIndex];
