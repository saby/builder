...
async function cleanMissingModules(
   ...
   const missingModules = outputModules.filter(outputModule => !currentModules.includes(outputModule));
   await pMap(
      missingModules,
      async(missingModule) => {
         ...
         if (isDirectory) {
            ...
            const cacheFilePath = path.join(taskParameters.config.cachePath, 'modules-cache', `${missingModule}.json`);
            ...
            taskParameters.config.addIntoGarbage(cacheFilePath);
         }
      }
   );
}
