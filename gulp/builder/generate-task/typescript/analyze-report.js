...
/**
 * Переключатель на время обкатки нового решения.
 * @type {boolean}
 */
const USE_NEW_ANALYZER = false;
...
function analyzeReport(taskParameters, logFile) {
   return async function analyzeReportTask() {
      ...
      forEachMessage(contents, (info) => {
         ...
         if (moduleInfo.typescript.typecheck || typescriptDiagnosticMessages.exceptional.has(info.code)) {
            ...
            if (typescriptDiagnosticMessages.message.has(info.code)) {
...
module.exports = USE_NEW_ANALYZER ? analyzeReport : require('./analyze-report-depr');
