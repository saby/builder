...
const { loadNoCheckMatcher } = require('./ts-no-check');
 ...
 */
function analyzeReport(taskParameters, logFile, allMessages, tscSeparate, projectReferences) {
   return async function analyzeReportTask() {
      ...
      const noCheckMatcher = await loadNoCheckMatcher(taskParameters.config.cachePath, taskParameters.config.modules);
      ...
      forEachMessage(contents, (info) => {
         ...
         // при сборке через project references любая ошибка не позволит скомпилировать код, поэтому всегда ругаемся
         // с уровнем ERROR и роняем сборку с уровнем критической ошибки.
         if (projectReferences) {
            logger.error({
               message: info.raw,
               moduleInfo
            });
            overallCode = CRITICAL_ERROR_CODE;
         } else if (moduleInfo.typescript.typecheck || typescriptDiagnosticMessages.alwaysError.has(info.code)) {
            if (info.code === 2307) {
            ...
            if (info.code === 1149 || info.code === 2307) {
               // при tsc separate(только в online-ie осталась) данная проверка избыточна, она ругается на файлы только
               // потому что модули делятся на 2 кучи и в каждой куче не достаёт каких то зависимостей, а по факту они
               // существуют.
               if (tscSeparate) {
                  return;
               }
            ...
            const shouldNotCheck = noCheckMatcher(info.file.path);
            const shouldTreatAsWarning = (
               typescriptDiagnosticMessages.warning.has(info.code) ||
               (
                  typescriptDiagnosticMessages.error.has(info.code) &&
                  shouldNotCheck
               )
            );

            if (shouldTreatAsWarning) {
               totalContents.push(formattedMessage);
               ...
               if (taskParameters.config.ESVersion !== 5) {
                  // Не заполняем консоль большим количеством ошибок для IE сборок
                  logger.warning({
                     message: info.raw,
                     moduleInfo
                  });
            ...
            // в нестрогом конфиге есть пачка ошибок, которые в строгом конфиге неактуальны. В случае
            // использования данного режима компиляции игнорируем только некритичные ошибки, на все
            // критичные ошибки также продолжаем ругаться(обработка данных ошибок выше)
            if (
               !taskParameters.config.ignoreNonCriticalErrors &&
               typescriptDiagnosticMessages.error.has(info.code)
            ) {
