...
function getLogFilePath(taskParameters, output, projectReferences) {
   const logFolder = taskParameters.config.logFolder || cwd();

   return output || path.join(
      logFolder,
      `builder_compilation_errors${projectReferences ? '_for_project_references' : ''}.log`
   );
}
...
function generateTaskForTypescript(taskParameters, output) {
   ...
   function getTscTasksList() {
      ...
      let logFile = getLogFilePath(taskParameters, output, true);
      const allMessages = [];
      ...
      tscTasks.push(
         gulp.series(
            ...
            workspace.clean(taskParameters, true),
            analyzeReport(taskParameters, logFile, allMessages, [], true)
         )
      );
      ...
      // определяем путь до логов для обычной сборки
      logFile = getLogFilePath(taskParameters, output);
      ...
      excludeModulesLists.forEach((excludeModulesList) => {
         tscTasks.push(
            gulp.series(
               ...
               analyzeReport(taskParameters, logFile, allMessages, excludeModulesList.length > 0)
            )
         );
      });
