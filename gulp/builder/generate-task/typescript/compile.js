...
async function checkForTscCache(taskParameters, projectReferences) {
   if (taskParameters.config.tscCache) {
      const { tscCachePath } = taskParameters.config;

      // в project references кеш включён по умолчанию и корневого кеш-файла не существует, поэтому
      // в таком случае пропускаем проверку на кеш
...
function promiseExecTsc(taskParameters, options) {
   const {
      tscCompileError,
      logFile,
      projectReferences
   ...
   return new Promise((resolve) => {
      ...
      const tscArguments = [
         toPlatform(cliPath),
         '--compiler',
         projectReferences ? '--build' : '--project',
      ...
      // исследования
      // при использовании project references кеш работает по умолчанию и задавать опции не надо
      // это приведёт к ошибке
      if (!isDisabledCache && !projectReferences) {
