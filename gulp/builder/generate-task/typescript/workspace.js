...
function checkForTsInFilesWithErrors(taskParameters) {
   const filesWithErrors = taskParameters.cache.getFilesWithErrors();

   filesWithErrors.forEach((currentFile) => {
      if (currentFile.endsWith('.ts') || currentFile.endsWith('.tsx')) {
         logger.info('Обнаружены ts/tsx файлы с предыдущей сборки, которые были собраны с ошибками. tsc компилятор будет запущен для их пересборки');
...
function prepare(taskParameters) {
   return async function prepareTypescriptWorkspace() {
      checkForTsInFilesWithErrors(taskParameters);
