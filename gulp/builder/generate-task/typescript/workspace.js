...
function getExcludedModules() {
   const excludedModules = process.env.TSC_EXCLUDE_MODULES;

   if (typeof excludedModules === 'string') {
      return excludedModules.split(',');
   }
...
function prepare(taskParameters) {
   return async function prepareTypescriptWorkspace() {
      ...
      const excludeModulesList = getExcludedModules();
