...
const { createConfig, addWSCoreAliases } = require('./configuration');
...
// поскольку saby-typescript в node_modules может быть расположен в разных местах в зависимости от окружения
// запуска билдера, определяем его следующим образом по приоритетам:
// 1) берём saby-typescript, если нам был передан node_modules в конфигурацию сборки сам wasaby-cli.
// 2) если нет, берём saby-typescript из node_modules, если билдер сам является npm-зависимостью.
// 3) берём saby-typescript из node_modules самого билдера.
async function getTypescriptDirectory(taskParameters) {
   if (
      taskParameters.config.nodeModules &&
      await fs.pathExists(taskParameters.config.nodeModules)
   ) {
      logger.debug(`используем в качестве node_modules: ${taskParameters.config.nodeModules}`);
      return path.join(taskParameters.config.nodeModules, 'saby-typescript');
   }
...
function prepare(taskParameters, excludeModulesList = [], projectReferences) {
   return async function prepareTypescriptWorkspace() {
      ...
      if (projectReferences) {
         taskParameters.config.setProjectReferencesList(
            PROJECT_REFERENCES_MODULES.filter(
               moduleName => taskParameters.config.moduleInfoMap.hasOwnProperty(moduleName)
            )
         );
      }
      ...
      taskParameters.sabyTypescriptDir = await getTypescriptDirectory(taskParameters);
      ...
      const config = createConfig(taskParameters, projectReferences);
      ...
      taskParameters.config.modules.forEach((moduleInfo) => {
         ...
         if (projectReferences) {
            ...
            if (PROJECT_REFERENCES_MODULES.includes(moduleInfo.name)) {
               ...
               // для WS.Core прописываем в алиасах путь до кеша tsc.
               if (moduleInfo.name === 'WS.Core') {
                  addWSCoreAliases(config.compilerOptions.paths, moduleInfo.typescriptOutputDir);
               }
               return;
            }
            try {
               ...
               const errorMessage = 'Не смогли создать симлинк на UI-модуль:\n' +
                  `source: ${moduleInfo.path}\n` +
                  `output: ${newPath}\n` +
                  'Убедитесь, что IDE или cli запущена с правами админа. Если это не помогает, перезагрузите IDE или терминал и повторите запуск сборки\n' +
                  `Error: ${err.message}`;

               // если симлинк уже был создан ранее, просто ругаемся предупреждением. Скорее всего
               // данный симлинк держится другим процессом и создавать его заново нет смысла.
      ...
      try {
         ...
         if (await fs.pathExists(additionalTypesPath)) {
            ...
            for (const typeName of typesToSymlink) {
               const outputPath = path.join(taskParameters.config.tscDirectory, `node_modules/@types/${typeName}`);
               if (await fs.pathExists(outputPath)) {
                  await fs.remove(outputPath);
               }

               // копируем типы для pixi и pixi-react в глобальный неймспейс типов для tsc компилятора
               // ВНИМАНИЕ! Нужно именно копировать, а не симличить типы, поскольку typescript ориентируется
               // не на расположение симлинка, а на оригинальное расположение файла
               logger.debug(`копируем библиотеку Typescript/types/${typeName} по пути typescript-sources/node_modules/@types/${typeName}`);
               await fs.copy(
                  path.join(taskParameters.config.sourcesDirectory, `Typescript/types/${typeName}`),
                  outputPath
               );
            }
         }
...
function clean(taskParameters, projectReferences) {
   return async function cleanTypescriptWorkspace() {
      ...
      if (projectReferences) {
         // исключаем ситуацию когда билдер удаляют(например при обновлении SDK или при переносе директории)
         // и симлинк становится битый
         await fs.remove(path.join(taskParameters.config.typescriptOutputDir, 'node_modules'));

         // чтобы при переиспользовании скомпиленных типов в основной проверке tsc компилятор понял, откуда
         // брать типы(например для react), необходимо засимличить в кеш этих модулей папку node_modules, как и при
         // запуске компилятора и подготовке окружения, поскольку кеш лежит в отдельной директории и компилятор не
         // видит типы конкретно для закешенных модулей.
         await fs.ensureSymlink(
            path.dirname(taskParameters.sabyTypescriptDir),
            path.join(taskParameters.config.typescriptOutputDir, 'node_modules')
         );
      }
