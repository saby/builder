      ...
      try {
         ...
         if (await fs.pathExists(additionalTypesPath)) {
            ...
            for (const typeName of typesToSymlink) {
               ...
               const libraryToCopy = path.join(taskParameters.config.sourcesDirectory, `Typescript/types/${typeName}`);
               logger.debug(`копируем библиотеку ${libraryToCopy} по пути ${outputPath}`);
               await fs.copy(
