...
const { getTailwindDuplicates, getTailwindReplacers } = require('../../../lib/tailwind/duplicates');
...
function writePrettyJSON(filePath, value) {
   return fs.writeJSON(filePath, value, { spaces: 3 });
}
   ...
   return async function buildCustomTailwind() {
      ...
      try {
         ...
         if (await fs.pathExists(cacheFilePath)) {
            ...
            const shouldApplyCachedData = (
               tailwindInfo.version === TW_CACHE_VERSION &&
               shouldOnlyLoadFromCache(taskParameters, moduleInfo)
            );
...
async function writeDuplicatesArtifact(taskParameters, table) {
   const possibleStatic = { };
   const dynamic = { };

   table.forEach((entity) => {
      const dst = entity.isPossibleStatic ? possibleStatic : dynamic;

      const key = entity.ruleSeq.join(' ');

      dst[key] = entity.modules;
   });

   const cacheFilePath = path.join(getCacheDirectory(taskParameters), 'tw-duplicates.json');

   await writePrettyJSON(cacheFilePath, { possibleStatic, dynamic });
}

async function writeReplacersArtifact(taskParameters, twReplacersSeq) {
   const twReplacersObj = { };

   twReplacersSeq.forEach((replacers, module) => {
      twReplacersObj[module] = replacers;
   });

   const cacheFilePath = path.join(getCacheDirectory(taskParameters), 'tw-replacers.json');

   await writePrettyJSON(cacheFilePath, twReplacersObj);
}

function getTailwindInfoSeq(tailwindModule, sourceModules) {
   const tailwindInfoSeq = sourceModules
      .map(moduleInfo => moduleInfo.tailwindInfo)
      .filter(v => (v !== null && v !== undefined));

   tailwindInfoSeq.unshift({
      version: TW_CACHE_VERSION,
      module: tailwindModule.outputName,
      dependency: `css!${tailwindModule.outputName}/tailwind`,
      outputFile: path.join(tailwindModule.path, 'tailwind.css'),
      snapshot: tailwindModule.tailwindCssSnapshot
   });

   return tailwindInfoSeq;
}

function generateTaskForProcessDuplicates(taskParameters, tailwindModule, sourceModules) {
   return async function processTailwindDuplicates() {
      try {
         const tailwindInfoSeq = getTailwindInfoSeq(tailwindModule, sourceModules);
         const table = getTailwindDuplicates(tailwindInfoSeq);

         await writeDuplicatesArtifact(taskParameters, table);

         const twReplacersSeq = getTailwindReplacers(table);

         await writeReplacersArtifact(taskParameters, twReplacersSeq);

         sourceModules.forEach((moduleInfo) => {
            if (twReplacersSeq.has(moduleInfo.name)) {
               moduleInfo.tailwindInfo.replacers = twReplacersSeq.get(moduleInfo.name);
            }
         });
      } catch (error) {
         // TODO: включить после тестирования
         logger.debug({
            message: `Error processing compiled tailwind modules: ${error.message}. ${error.stack}`,
            error
         });
      }
   };
...
function generateTaskForBuildTailwindCss(taskParameters) {
   ...
   // Не используем разделение на { build, skip }, потому что
   // самостоятельно обрабатываем коллекцию измененных файлов для модулей
   ...
   return gulp.series(
      ...
      generateTaskForProcessDuplicates(taskParameters, tailwindModule, sourceModules),
