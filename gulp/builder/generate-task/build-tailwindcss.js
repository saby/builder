...
function generateTaskForScanningTailwind(taskParameters, tailwindModule) {
   ...
   return async function scanningTailwindModule() {
      try {
         let tailwindCssContents;
         try {
            const filePath = path.join(tailwindModule.path, 'tailwind.css');
            tailwindCssContents = await fs.readFile(filePath, 'utf8');
         } catch (err) {
            const dirList = await fs.readdir(tailwindModule.path);

            logger.error({
               message: `Error loading Tailwind/tailwind.css: ${err.message}. Tailwind directory list: ["${dirList.join('","')}"]`,
               moduleInfo: tailwindModule,
               error: err
            });
         }
