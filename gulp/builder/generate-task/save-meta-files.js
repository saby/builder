      ...
      try {
         ...
         if (metaTsFiles.length > 0) {
            await pMap(
               ...
               async(metaTsFile) => {
                  ...
                  if (error) {
                     ...
                     if (metaTs instanceof Array) {
                        result.meta.push(...metaTs);
                     } else {
                        result.meta.push(metaTs);
                     }
                  }
            ...
            if (filesWithErrors.length > 0) {
               const errorsCachePath = path.join(taskParameters.config.cachePath, `modules-cache/${moduleInfo.outputName}.json`);
