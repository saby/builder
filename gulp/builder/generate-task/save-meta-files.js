 ...
 * needed for incremental build of versioned_modules
 * meta files
...
// versioned_modules содержит в себе только информацию из стилей, js-кода и html-страниц.
...
function generateTaskForSaveMetaFiles(taskParameters) {
   ...
   return gulp.series(
      ...
      gulp.parallel(tasks),
      (done) => {
         taskParameters.resetVersionedMeta();
         done();
      },
      buildModule.finish()
   );
