 ...
 * Example: for interface module "Controls" with theme name "online" interface
...
function parseThemeName(modulesList, currentModuleNameParts) {
   ...
   if (currentModuleParts.length >= 2) {
      ...
      while (!interfaceModuleParsed && currentModuleParts.length > 0) {
         const currentPart = currentModuleParts.pop();
         themeNameParts.unshift(currentPart);

         if (approvedThemes.has(themeNameParts.join('-'))) {
            interfaceModuleParsed = currentModuleParts.join('-');
            break;
         }
...
function generateTaskForMarkThemeModules(taskParameters) {
   ...
   const processFallbacks = function processFallbacks(done) {
      Object.values(fallbackDeclarations).forEach((decl) => {
         ...
         taskParameters.addFilesToCopy(moduleInfo.outputName, ['fallback.json']);
      });
