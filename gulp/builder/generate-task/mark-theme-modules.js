...
function generateTaskForMarkThemeModules(taskParameters) {
   ...
   const tasks = modulesWithThemes.map((moduleInfo) => {
      ...
      // variables less needed to parse only in IE build
      // required for default css variables values
   ...
   const processFallbacks = async function processFallbacks() {
      ...
      // для Controls-default-theme перед сборкой less необходимо подменить content
      // иконки в зависимости от региона, поэтому его надо подмешивать именно в исходник.
      // TODO после перевода cbuc-icons на автоматический генератор шрифта данный костыль можно
      //  будет удалить
      const glyphsSourcePath = path.join(taskParameters.config.sourcesDirectory, 'Controls-default-theme/fonts/_glyphs.less');
      const currentRegion = taskParameters.config.countries[0];
      if (currentRegion === 'KZ' || currentRegion === 'UZ') {
         const test = await fs.readFile(glyphsSourcePath, 'utf8');
         await fs.outputFile(
            glyphsSourcePath,
            test
               .replace('.icon-currentCurrency::before {\n' +
                  '   content: "\\e6a3";\n' +
                  '}', '.icon-currentCurrency::before {\n' +
                  `   content: "${CURRENCY_GLYPH_MAP[currentRegion]['\\e6a3']}";\n` +
                  '}')
               .replace('.icon-currentCurrencyNone::before {\n' +
                  '   content: "\\e9cd";\n' +
