...
const { createSVG } = require('svgtofont/lib/utils');
...
async function getProcessedCssText(moduleInfo, fontName, currentFile, versionEnabled, cssText, fontRoot, cdnUrl) {
   ...
   processedCssStyle = await versionizeStyles(
      ...
      moduleInfo,
      { ignoreQueryParams: true, fontRoot, cdnUrl }
   );
...
async function saveFontCss(taskParameters, moduleInfo, to, currentFile, processedCssStyle, minifiedCss) {
   ...
   taskParameters.addVersionedModules(moduleInfo.outputName, [`${moduleInfo.outputName}/${currentFile}`, `${moduleInfo.outputName}/${minCurrentFile}`]);
   taskParameters.addFilesToCopy(moduleInfo.outputName, [currentFile, minCurrentFile]);
}
   ...
   directoryList.forEach((currentFile) => {
      ...
      if (fileName === fontName && INCLUDE_EXTENSIONS.has(extension)) {
         if (extension === '.css') {
            ...
            taskParameters.addFilesToCopy(moduleInfo.outputName, [currentFile]);
...
// Данная функция генерирует мапу-соответствие имени иконки и её юникода
// TODO Эта функция является временным решением до внедрения в сборки Node.JS 18+. В пакете svgtofont
//  уже есть данный функционал под опцией generateInfoData, начиная с версии 3.21.0
//  https://github.com/jaywcjlove/svgtofont/releases/tag/v3.21.0
//  но обновиться до неё мы пока не можем из-за несовместимых зависимостей(cheerio, требует Node.JS 18)
//  Поэтому пока генерируем json-мапу своими силами
async function getUnicodeMap(options) {
   const infoData = {};
   const unicodeObject = await createSVG(options);
   Object.keys(unicodeObject).forEach((iconName) => {
      if (!infoData[iconName]) {
         infoData[iconName] = {};
      }

      const currentCode = unicodeObject[iconName];
      let encodedCodes = currentCode.charCodeAt(0);
      if (options.useNameAsUnicode) {
         encodedCodes = currentCode.split('').map(x => x.charCodeAt(0)).join(';&amp;#');
      }
      infoData[iconName] = `\\${encodedCodes.toString(16)}`;
   });
...
async function generateFontByOutput(taskParameters, moduleOutput, currentFont, fontsToGenerate, moduleInfo) {
   ...
   try {
      ...
      try {
         ...
         const currentBuildLogs = await fs.readFile(fontBuildLogsPath, 'utf8');

         // поскольку у данной библиотеки ошибки тупо пишутся в лог и не выбрасываются в виде критической ошибки
         // https://github.com/jaywcjlove/svgtofont/blob/master/src/index.ts#L445
         // будем выбрасывать ошибку сами
         if (currentBuildLogs.includes('SvgToFont:CLI:ERR:')) {
            throw new Error(
               currentBuildLogs.slice(
                  currentBuildLogs.indexOf('SvgToFont:CLI:ERR:'),
                  currentBuildLogs.indexOf('done!'),
               )
            );
         }
      } catch (error) {
      ...
      // TODO подумать как лучше сделать, в модуле WNC-icons разрабы назвали свой шрифт "icons"
      //  и мы формировали мапу под именем "icons.json", но это имя зарезервировано билдером под
      //  соответствующий метафайл, который поставляет описание SVG-пакетов.
      const currentUnicodeMapPath = path.join(moduleOutput, `${currentFont}-map.json`);
      try {
         const currentUnicodeMap = await getUnicodeMap(svgOptions);
         await fs.outputJSON(currentUnicodeMapPath, currentUnicodeMap);
      } catch (error) {
         logger.warning({
            message: 'Ошибка при формировании JSON-списка иконок и их юникодов',
            error,
            moduleInfo,
            filePath: currentUnicodeMapPath
         });
      }
   ...
   return async function buildIconsFont() {
      ...
      for (const currentFont of Object.keys(fontsToGenerate)) {
         ...
         if (fontsToGenerate[currentFont].region) {
            await pMap(
               fontsToGenerate[currentFont].region,
               async(currentRegion) => {
                  await generateFontByOutput(
                     taskParameters,
...
function generateTaskForCacheCheck(moduleInfo) {
   return function checkIconsModuleForChanges(done) {
      if (helpers.moduleHasNoChanges(moduleInfo, [moduleInfo.svgChanged])) {
