...
function generateTaskForBuildFont(taskParameters, moduleInfo, fontsToGenerate) {
   return async function buildIconsFont() {
      for (const currentFont of Object.keys(fontsToGenerate)) {
         const fontOutput = `${moduleInfo.output}/temp-${currentFont}`;
         const configOutput = `${moduleInfo.output}/temp-${currentFont}-config`;
         try {
            logger.debug(`Generating font for "${fontsToGenerate[currentFont]}" svg files`);
            const svgOptions = {
               src: fontsToGenerate[currentFont],
               dist: fontOutput,
               startUnicode: 0xea01,
               ...
               website: {
                  ...
                  title: `Generated font for svg files in "${currentFont}" folder`
               }
            };
            ...
            const relativeSource = path.relative(moduleInfo.output, fontsToGenerate[currentFont]);
            try {
               await exec(
                  `node ${require.resolve('svgtofont/lib/cli')} --sources ../${relativeSource} --output ../temp-${currentFont}`,
                  processOptions
               );
            } catch (error) {
            ...
            await moveCommonFontArtifacts(currentFont, fontOutput, moduleInfo.output);
         } catch (error) {
            logger.warning({
               message: 'Error generating font',
               error,
