...
function getFontSizeValue(currentFont, buildIE) {
...
async function getProcessedCssText(moduleInfo, currentFile, versionEnabled, cssText, fontRoot) {
   if (!versionEnabled) {
      return cssText;
   }
   ...
   const processedCssStyle = await versionizeStyles(
      new PosixVinyl({
 ...
 */
async function moveCommonFontArtifacts(taskParameters, moduleInfo, fontName, from, to) {
   const promises = [];
         ...
         if (extension === '.css') {
            const moveAndMinifyStyle = async() => {
               ...
               const processedCssStyle = await getProcessedCssText(
                  ...
                  cssStyle,
                  taskParameters.config.buildIE ? from : ''
               );

               const minifiedCss = minifyCss(processedCssStyle);
...
async function generateFontByOutput(taskParameters, moduleOutput, currentFont, fontsToGenerate, moduleInfo, region) {
   ...
   try {
      ...
      const fontSize = getFontSizeValue(currentFont, taskParameters.config.buildIE);
      const svgOptions = {
      ...
      // где используется сервис статики и сервис представления. В остальных случаях
      // иконку надо оставлять с относительным путём, она сама подгрузится браузером.
      if (taskParameters.config.version && taskParameters.config.multiService) {
      ...
      await moveCommonFontArtifacts(taskParameters, moduleInfo, currentFont, fontOutput, moduleOutput);
   } catch (error) {
      logger.warning({
