...
function generateTaskForSingleModule(taskParameters, moduleInfo) {
   ...
   return function buildStaticTmplJson(done) {
      // нет нужды генерировать static_templates.json, если html.tmpl компоненты не менялись
      if (moduleHasNoChanges(moduleInfo, [moduleInfo.htmlTmplChanged])) {
         return done();
      }
