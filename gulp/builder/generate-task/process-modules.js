      ...
      await pMap(
         ...
         async(moduleInfo) => {
            ...
            if (moduleInfo.changedFiles && moduleInfo.changedFiles.length > 0) {
               ...
               const cacheFilePath = path.join(taskParameters.config.cachePath, 'modules-cache', `${moduleInfo.outputName}.json`);
               const stableCacheFilePath = path.join(taskParameters.config.cachePath, 'modules-cache', `${moduleInfo.outputName}_stable.json`);

               if (await fs.pathExists(stableCacheFilePath)) {
                  await fs.remove(cacheFilePath);
                  await fs.move(stableCacheFilePath, cacheFilePath);
               }
   ...
   return async function saveStableModules() {
      await pMap(
         ...
         async(moduleInfo) => {
            ...
            // отработки патча. Как итог - мы сможем делать сколько угодно патчей на одном stable билде и не будем
            // получать такие конфликты.
            // Тоже самое повторяем с кешем конкретного модуля.
            const stableCompiledModulePath = path.join(compiledRoot, `${moduleInfo.outputName}_stable`);
            const cacheFilePath = path.join(taskParameters.config.cachePath, 'modules-cache', `${moduleInfo.outputName}.json`);
            const stableCacheFilePath = path.join(taskParameters.config.cachePath, 'modules-cache', `${moduleInfo.outputName}_stable.json`);

            if (await fs.pathExists(cacheFilePath)) {
               await fs.copy(cacheFilePath, stableCacheFilePath);
            }
            await execInPool(
               taskParameters.pool,
               'doAsyncFs',
