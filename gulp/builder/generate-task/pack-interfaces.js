...
const uglifyJs = require('../../../lib/run-uglify-js');
  ...
  function getRootContents() {
   ...
   if (global.contents) {
      if (typeof module === 'object' && typeof module.exports === 'object') {
         defineModuleWithContents(global.contents, ${!!data.generateUMD});
      } else {
...
async function updateContents(taskParameters, outputDir, newModuleContents) {
...
async function processFacade(taskParameters, output, metaForUpdate) {
   ...
   await pMap(interfaces.required, async(interfaceName) => {
      ...
      // builder should update contents.json meta about features only in single-service application(apps
      // built with wasaby-cli and desktop applications)
   ...
   if (shouldUpdateContents) {
      await updateContents(taskParameters, output, newModuleContents);
   }
}
