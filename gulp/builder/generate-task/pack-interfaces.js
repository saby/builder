...
${rmEndingNewLines(data.originFacadeContent)}
   ...
   function defineModuleWithContents(currentContents, isUMD) {
      if (!currentContents) {
         ...
         if (currentModuleContents.features && currentModuleContents.features[currentFeatureName]) {
            if (
               currentModuleContents.features[currentFeatureName] &&
               currentModuleContents.features[currentFeatureName].includes('/')
            ) {
               currentProvider = currentModuleContents.features[currentFeatureName];
            } else {
               currentProvider = currentModuleContents.features[currentFeatureName] + '/' + currentInterfaceParts.join('/');
            }
  ...
  function getRootContents() {
   ...
   var currentFeatureName = "${data.currentFeatureName}";
...
async function createFacadeCode(output, options) {
   const {
      currentFeatureName,
      currentInterface,
      interfaceModuleName,
...
// FIXME: имеем ошибку
//   Gulp: [RouterDemo/StaticTemplate/staticFull.html.tmpl] Ошибка при обработке html-tmpl шаблона:
//   promiseInfo.domain.emit is not a function
// function generateContents(modules) {
//    // FIXME: текущая положение дел в сборке требует,
//    //  чтобы окружение было инициализировано полным списком модулей.
//    //  Необходимо подумать, как наилучшим образом это требование реализовать.
//    const contents = { modules: { } };
//
//    // modules.forEach((moduleInfo) => {
//    //    contents.modules[moduleInfo.outputName] = {};
//    // });
//
//    return contents;
// }
...
function getPreparedProviders(currentInterface, interfaces) {
   return Object.keys(interfaces.provided)
      .filter(
         currentKey => currentKey === currentInterface.name
      )
      .sort((first, second) => {
         const firstIndex = interfaces.providedOrder.findIndex(first);
         const secondIndex = interfaces.providedOrder.findIndex(second);

         return firstIndex - secondIndex;
      }).map(
         currentKey => interfaces.provided[currentKey]
      );
}
...
async function processFacade(taskParameters, output, facadeList) {
   ...
   const interfaceModulesCache = {};

   await pMap(interfaces.required, async(currentFacade) => {
      const moduleInfo = taskParameters.config.getModuleInfoByName(currentFacade.path);
      const cachePath = path.join(moduleInfo.output, '.cache/components-info.json');
      const data = await fs.readJson(cachePath);
      interfaceModulesCache[moduleInfo.name] = {
         path: cachePath,
         data
      };
   }, P_MAP_OPTIONS);
   ...
   await pMap(interfaces.required, async(currentFacade) => {
      const interfaceName = currentFacade.path;
      const moduleInfo = taskParameters.config.getModuleInfoByName(interfaceName);
      const providers = getPreparedProviders(currentFacade, interfaces);
      ...
      const [, callbackName] = lastProvider.split('/');
      ...
      newModuleContents[moduleInfo.outputName].features[currentFacade.name] = lastProvider;

      // прокидываем в кеш зависимость от провайдера, чтобы анализатор мог находить циклы по провайдерам
      if (lastProvider !== interfaceName) {
         const componentName = `${interfaceName}.ts`;
         if (!interfaceModulesCache[moduleInfo.name].data.componentsInfo[componentName]) {
            logger.warning({
               message: `Не нашли в components-info информацию о зависимостях интерфейса ${componentName}`
            });
         } else {
            interfaceModulesCache[moduleInfo.name].data.componentsInfo[componentName].componentDep.push(lastProvider);
         }
      }
      ...
      if (taskParameters.config.joinedMeta) {
         ...
         currentModuleContents.features[currentFacade.name] = lastProvider;
      }
      ...
      if (
         ...
         const debugFacadeCode = await createFacadeCode(
            output,
            {
               currentFeatureName: currentFacade.name,
               currentInterface: interfaceName,
               interfaceModuleName: moduleInfo.outputName,
               callbackName,
               extension: '.origin.js',
               generateUMD
            }
         ...
         if (isReleaseMode) {
            const releaseFacadeCode = await createFacadeCode(
               output,
               {
                  currentFeatureName: currentFacade.name,
                  currentInterface: interfaceName,
            ...
            try {
               ...
               logger.error({
                  ...
                  error,
                  moduleInfo,
                  filePath: `${interfaceName}.min.origin.js`
               });
   ...
   await pMap(Object.keys(interfaceModulesCache), async(moduleName) => {
      await fs.outputJson(interfaceModulesCache[moduleName].path, interfaceModulesCache[moduleName].data);
   }, P_MAP_OPTIONS);
