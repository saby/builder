...
class ModuleCache {
   ...
   getCurrentFailedFiles() {
      return this.currentStore.filesWithErrors || [];
   }
...
function generateReadModuleCache(taskParameters, moduleInfo, singleFileBuild) {
   ...
   return async function readModuleCache() {
      ...
      if (taskParameters.cache.isCacheNeeded()) {
         const lockFilePath = withLockfile.toFileName(taskParameters, moduleInfo.outputName);
      ...
      if (!taskParameters.cache.grabber) {
         ...
         taskParameters.cache.storeFailedFiles(moduleInfo.name, moduleInfo.cache.getFailedFiles());
      }
   };
}
