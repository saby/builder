...
class ModuleCache {
   ...
   constructor(lastStore) {
      this.markupProperties = ['text', 'nodeName', 'dependencies', 'versioned'];
   ...
   addMetaTypesFile(filePath) {
      this.currentStore.metaTypesFiles.push(filePath);
   }

   getMetaTypesList() {
      return this.currentStore.metaTypesFiles || [];
   }

   migrateMetaTypesList() {
      this.currentStore.metaTypesFiles = this.lastStore.metaTypesFiles;
   }
...
function generateWriteModuleCache(taskParameters, moduleInfo) {
   return async function writeModuleCache() {
      ...
      try {
         ...
         // перед тем как удалим помодульный кеш, вытащим из него мета-данные о списке файлов метатипов,
         // он нам потом пригодится при генерации метатипов.
         moduleInfo.metaTypesFiles = moduleInfo.cache.getMetaTypesList();
