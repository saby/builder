...
class StoreInfo {
   ...
   async load(cachePath, modules, watcherRunning) {
      const builderInfoExists = await fs.pathExists(path.join(cachePath, 'builder-info.json'));

      if (builderInfoExists) {
         if (!watcherRunning) {
         ...
         try {
            ...
            this.sabyPageProvider = builderInfo.sabyPageProvider;
            if (!watcherRunning) {
   ...
   async save(cacheDirectory, logFolder, modules) {
      await fs.outputJson(
         path.join(cacheDirectory, 'builder-info.json'),
         {
            ...
            sabyPageProvider: this.sabyPageProvider
         },
         {
            spaces: 1
         }
   ...
   getOutputFilesSetForDeletedFiles(deletedFiles, isFirstBuild) {
      ...
      for (const module in this.inputPaths) {
         ...
         deletedFiles.forEach((currentPath) => {
            if (this.inputPaths[module].paths.hasOwnProperty(currentPath)) {
               ...
               if (!fileHasCollisions) {
                  for (const relativeFilePath of this.inputPaths[module].paths[currentPath].output) {
                     ...
                     if (!extname) {
                        // Произошел случай, когда в качестве extname получили пустую строку,
                        // из-за чего возникла синтаксическая ошибка RegEx.
                        // Нужно постараться хотя бы поймать такой случай в логах
                        logger.debug(`Got no extension for file ${relativeFilePath}`);
            ...
            // выхлоп самого UI-модуля надо тоже проанализировать, туда попадают различные мета-данные
            // для сервиса представления(routes-info.json к примеру)
            if (this.inputPaths[module].output.includes(currentPath)) {
               resultSet.add(currentPath);
            }
