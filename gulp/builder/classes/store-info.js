...
class StoreInfo {
   ...
   async saveOutputModuleCache(modules) {
      ...
      await pMap(
         ...
         async(moduleInfo) => {
            ...
            if (dependenciesCacheByModules[moduleInfo.outputName]) {
               await fs.outputJson(
                  path.join(moduleInfo.output, '.cache/dependencies.json'),
                  sortObject(dependenciesCacheByModules[moduleInfo.outputName])
               );
            }
         },
         { concurrency: 50 }
   ...
   async load(cachePath, modules, watcherRunning) {
      if (await fs.pathExists(path.join(cachePath, 'builder-info.json'))) {
         ...
         try {
            this.reactMode = (await fs.readJson(path.join(cachePath, 'react-mode.json'))).react;
         } catch (error) {
            if (!watcherRunning) {
               logger.debug({
                  message: `Cache file "${path.join(cachePath, 'react-mode.json')}" failed to be read`,
                  error
               });
            }
         }
