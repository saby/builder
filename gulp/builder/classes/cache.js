...
class Cache {
      ...
      if (cacheHasIncompatibleChanges) {
         ...
         if (await fs.pathExists(this.config.cachePath)) {
            for (const fileName of await fs.readdir(this.config.cachePath)) {
               if (!(fileName.endsWith('.lockfile') || ['temp-modules', 'typescript-cache', 'lock-files'].includes(fileName))) {
   ...
   getFilesWithErrors(modulePath, transmittedStorage) {
      ...
      storeToCheck.filesWithErrors.forEach((currentPath) => {
         // TODO Сделать files-with-errors мета-данные помодульными,
         //  чтобы при анализе этих данных в get-module-input взять сразу список
         //  для конкретного интерфейсного модуля
         //  https://online.sbis.ru/opendoc.html?guid=8678bd92-099f-45ff-b721-4a09bad59efd&client=3
