...
class Cache {
   ...
   storeSabyPageProviderHash() {
      this.currentStore.sabyPageProviderHash = this.config.sabyPageProviderHash;
   }
   ...
   async load(changedFilesWithDependencies) {
      ...
      this.storeSabyPageProviderHash();
   ...
   checkForForceModuleRebuild(config) {
      ...
      currentRunningParameters.modules.forEach((currentModule) => {
         ...
         if (lastModule) {
            ...
            try {
               ...
               const moduleInfo = config.getModuleInfoByName(currentModule.name);
               moduleInfo.forceRebuild = true;
            }
         }
      });
      ...
      if (cacheHasIncompatibleChanges) {
         ...
         if (await fs.pathExists(this.config.cachePath)) {
            for (const fileName of await fs.readdir(this.config.cachePath)) {
               if (!(fileName.endsWith('.lockfile') || ['temp-modules', 'typescript-cache', 'lock-files', 'builder-extra-config.json', 'tailwind-cache', 'pagex-cache'].includes(fileName))) {
         ...
         this.checkForForceModuleRebuild(this.config);
   ...
   async checkForDropCacheCases(prettyPath, prettyRelativePath) {
      if (!getBuildStatusStorage().cacheIsDropped) {
         ...
         // json-файлы не участвуют в работе шаблонизаторов, поэтому не будем сбрасывать кеш просто так
         if (prettyPath.endsWith('.json')) {
            return;
         }
   ...
   addCurrentModuleThemesMeta(moduleInfo, changedThemes) {
      ...
      if (!changedThemes) {
         ...
         themesToMigrate.forEach((currentTheme) => {
            if (!normalizedDeletedFiles.includes(`${currentTheme}.less`)) {
               this.migrateCurrentTheme(moduleInfo, currentTheme);
            }
         });
   ...
   migrateNotChangedFiles(moduleInfo, config) {
      ...
      if (changedFiles) {
         ...
         if (!moduleInfo.metaTypesChanged) {
            moduleInfo.cache.migrateMetaTypesList();
         }
   ...
   storeLocalModuleDependencies(obj) {
      ['links', 'nodes', 'packedLibraries', 'lessDependencies'].forEach((currentProperty) => {
         if (obj[currentProperty]) {
            Object.keys(obj[currentProperty])
               .forEach((currentParameter) => {
                  this.moduleDependencies[currentProperty][currentParameter] = obj[currentProperty][currentParameter];
               });
         }
      });
   }
