 ...
 * @type {RegExp}
 */
const CACHED_FILES_EXTENSIONS = /(\.less)|(\.js)|(\.es)|(\.tsx?)$/;
...
class Cache {
   ...
   async load(changedFilesWithDependencies) {
      ...
      if (this.config.dropCacheForMarkupPath) {
         await this.checkTemplatesCompilerFile(
      ...
      if (cacheHasIncompatibleChanges) {
         ...
         if (await fs.pathExists(this.config.cachePath)) {
            for (const fileName of await fs.readdir(this.config.cachePath)) {
               if (!(fileName.endsWith('.lockfile') || ['temp-modules', 'typescript-cache', 'lock-files', 'react-mode.json'].includes(fileName))) {
   ...
   async _isFileChanged(moduleName, hashByContent, root, prettyRelativePath, prettyPath, hash) {
      ...
      // файл с ошибкой
      if (this.isFileFailed(prettyRelativePath)) {
         return true;
      }
   ...
   addRegionNode(currentCssNode, region, key, value) {
      if (!this.currentStore.regionNodes[currentCssNode]) {
         this.currentStore.regionNodes[currentCssNode] = {};
      }

      if (!this.currentStore.regionNodes[currentCssNode][region]) {
         this.currentStore.regionNodes[currentCssNode][region] = {};
      }
      this.currentStore.regionNodes[currentCssNode][region][key] = value;
   }

   getRegionNode(currentCssNode, region) {
      return this.currentStore.regionNodes[currentCssNode][region];
   }

   getRegionNodes() {
      return this.currentStore.regionNodes;
   }

   migrateRegionNode(currentCssNode, region) {
      if (this.lastStore.regionNodes[currentCssNode]) {
         if (!this.currentStore.regionNodes[currentCssNode]) {
            this.currentStore.regionNodes[currentCssNode] = {};
         }

         this.currentStore.regionNodes[currentCssNode][region] = this.lastStore.regionNodes[currentCssNode][region];
      }
   }
   ...
   isFileFailed(relativePath) {
      ...
      return this.currentStore.filesWithErrors[moduleName].has(relPath);
   }
