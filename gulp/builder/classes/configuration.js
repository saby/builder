...
class BuildConfiguration extends BaseConfiguration {
   constructor() {
      ...
      // generate tailwind css file based on usage of style classes.
      // Выключаю сборку tailwind, потому что сейчас сборка стилей происходит вхолостую:
      // - обрабатываются более 1000 модулей в основных сборках и локальных стендах;
      // - документация по адаптивности еще не выпущена -- по ней будем анализировать только те модули,
      //    в которых явно используются конструкции, не содержащиеся в статически собранном css файле.
      // Включить сборку по задаче, о чем ранее было совещание:
      // https://online.sbis.ru/opendoc.html?guid=3885811d-18b6-4c96-b3b4-0471dcecc824&client=3
      this.buildTailwindCss = false;
   ...
   generateConfig(symlinksExist, watcherChangedFiles) {
    ...
    */
   loadSync(argv) {
      const { config, symlinksExist, watcherChangedFiles } = ConfigurationReader.getProcessParameters(argv);
      super.readConfigFile(config);

      this.generateConfig(symlinksExist, watcherChangedFiles);
   }
   ...
   _installWatcherChangedFiles(watcherChangedFiles) {
      if (typeof watcherChangedFiles !== 'string') {
         return;
      }

      if (!fs.existsSync(watcherChangedFiles)) {
         return;
      }

      try {
         const changedFiles = fs.readJSONSync(watcherChangedFiles);
         for (const module of this.rawConfig.modules) {
            if (changedFiles[module.name]) {
               module.changedFiles = changedFiles[module.name].changedFiles;
               module.deletedFiles = changedFiles[module.name].deletedFiles;
            }
         }

         this.watcherRunning = true;
         this.watcherChangedFiles = true;
         this.emitTypescript = false;
      } catch (error) {
