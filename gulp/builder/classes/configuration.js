...
function getProvidedMeta(feature) {
   const nameParts = feature.path.split('/');

   // если задан кастомный path, то возвращаем в качестве featureName поле name, в
   // противном случае именем фичи является оставшяяся часть поля path(в path по
   // умолчанию пишется имя фичи)
   return [
      nameParts.shift(),
      feature.normalizedFeatureName ? feature.name : nameParts.join('/')
   ];
}
...
class BuildConfiguration extends BaseConfiguration {
   constructor() {
      ...
      // build project for use with static service. false by default
      this.staticService = false;
   }
   ...
   generateConfig(watcherChangedFiles) {
      ...
      if (this.rawConfig.hasOwnProperty('builderTests')) {
         ...
         // временный костыль, чтобы сборщики могли включить тесты по веткам online-inside без компиляции tsc
         // через переменную окружения
         // TODO сделать нормальный флаг на уровне jinnee по задаче
         //  https://online.sbis.ru/opendoc.html?guid=8dbd8285-3164-4ce9-9bf7-031a7a293cad&client=3
         if (process.env.DISABLE_TSC === 'true') {
            this.emitTypescript = false;
         }
      }

      if (this.rawConfig.hasOwnProperty('cdnUrl')) {
         this.cdnUrl = this.rawConfig.cdnUrl;
      }
   ...
   // проверяет соответствие переданной фичи фасаду
   // Проверку можно использовать в 2 сценариях:
   // 1) При анализе фич в конфигураторе билдера - здесь передаётся и путь до фичи и её имя
   // и проверяется полное соответствие как пути, так и имени, поскольку путь фичи может быть
   // кастомным и нужно проверить, чтобы имя фичи с кастомным путём соответствовало имени фасада, ведь
   // в модуле с фичей может лежать и его провайдер под кастомным путём
   // 2) При анализе конкретного файла в пайплайне основного билда. В данном случае передаётся только
   // путь и если мы нашли фасад с подобным именем, возвращаем true.
   isFacade(currentKey) {
      const predictedInterfaceName = currentKey.normalizedFeatureName || currentKey.path;

      return this.interfaces.required.some(currentInterface => currentInterface.path === predictedInterfaceName &&
         (!currentKey.name || currentInterface.name === predictedInterfaceName));
   }

   // получить мету по интерфейсам
   getInterfaces() {
   ...
   _configureModules(flagsList) {
      ...
      this.templateBuilder = false;
      this.sabyPageProvider = false;
      ...
      let modulesHaveCriticalErrors = false;
      for (const module of this.rawConfig.modules) {
         ...
         if (this._configureModuleFeatures(moduleInfo)) {
            modulesHaveCriticalErrors = true;
         }
         ...
         switch (moduleInfo.name) {
            ...
            case 'SabyPage':
               if (moduleInfo.hash) {
                  this.sabyPageProviderHash = moduleInfo.hash;
               }
               this.sabyPageProvider = true;
               this.modules.push(moduleInfo);
               break;
      ...
      if (modulesHaveCriticalErrors) {
         throw new Error('При анализе модулей обнаружены критические ошибки');
      }
   ...
   _configureBuildOptions() {
      ...
      // отключаем строгий конфиг по запросу, например в тяжелом проекте с огромным количеством ошибок(online-inside)
      // это временное решение, в typescript в версии 5.6 и выше уже есть полноценное решение с отключением тайпчека на
      // уровне компилятора, но чтобы полноценно обновить компилятор, нам нужен Node.Js 18+
      // TODO нужно будет с этим разобраться
      if (process.env.DISABLE_STRICT_TSC === 'true') {
         this.noStricttsconfig = getTsConfigPath(this.rawConfig.tsconfig, this.configFile, this.nodeModules, false);
         this.noStricttsCompilerOptions = getCompilerOptions(this.noStricttsconfig);
         this.tsconfig = getTsConfigPath(this.rawConfig.tsconfig, this.configFile, this.nodeModules, true);
         this.ignoreNonCriticalErrors = true;
      } else {
         this.tsconfig = getTsConfigPath(this.rawConfig.tsconfig, this.configFile, this.nodeModules, true);
      }
   ...
   // fills interfaces meta with information about dependencies from s3mod-file
   _configureInterfacesMeta() {
      this.interfaces.providedOrder.forEach((currentKey) => {
         ...
         if (this.isFacade(currentKey)) {
            this.interfaces.provided[currentKey.normalizedFeatureName || currentKey.name] = currentKey.path;
         }

         providedModuleDepends.forEach((currentDependency) => {
            const predictableInterface = {
               name: `${currentDependency}/${featureName}`,
               path: `${currentDependency}/${featureName}`
            };

            if (this.isFacade(predictableInterface)) {
               this.interfaces.provided[predictableInterface.path] = currentKey.path;
            }
         });
      });
   }
   ...
   _configureModuleFeatures(moduleInfo) {
      // достаёт данные о фиче. Пока это промежуточный вариант, он поддерживает 3
      // варианта передачи фичи в конфиг билдера:
      // 1) "ParametersScope"
      // 2) { "name": "ParametersScope }
      // 3) { "name": "custom_feature", "path": "ParametersScope" }
      // TODO после завершения перехода на новую архитектуру в jinnee и wasaby-cli удалить ненужный
      //  код совместимости
      const getFeatureMeta = (feature) => {
         if (typeof feature === 'string') {
            return {
               name: `${moduleInfo.outputName}/${feature}`,
               path: `${moduleInfo.outputName}/${feature}`
            };
         }

         // если в качестве фичи просто задан обьект с именем, то имя фичи автоматически становится путём до неё
         // а также имя фичи становится уникальным и привязанным к конкретному интерфейсному модулю
         if (!feature.path) {
            return {
               name: `${moduleInfo.outputName}/${feature.name}`,
               path: `${moduleInfo.outputName}/${feature.name}`
            };
         }

         // если в качестве фичи помимо задан обьект с именем и собственным путём, то имя фичи
         // становится глобальным на весь проект
         return {
            name: feature.name,
            path: `${moduleInfo.outputName}/${feature.path}`,
            normalizedFeatureName: `${moduleInfo.outputName}/${feature.name}`
         };
      };

      let hasCriticalError = false;
      if (moduleInfo.featuresRequired.length > 0) {
         moduleInfo.featuresRequired.forEach((requiredFeature) => {
            const currentFeature = getFeatureMeta(requiredFeature);

            // если в проекте уже есть фича с данным именем, мы должны упасть с ошибкой, поскольку
            // фичу могут назвать одним и тем же именем, но базовые интерфейсы могут быть разные, такое
            // допускать нельзя.
            const existingInterface = this.interfaces.required.find(
               currentInterface => currentInterface.name === currentFeature.name
            );

            if (existingInterface) {
               logger.error({
                  message: `В проекте уже существует базовый интерфейс с именем ${currentFeature.name} : ` +
                     `${JSON.stringify(existingInterface)}. Пытаемся добавить: ${JSON.stringify(currentFeature)}`,
                  moduleInfo
               });
               hasCriticalError = true;
            }

            if (currentFeature.path && !currentFeature.name) {
               logger.error({
                  message: 'Обнаружен некорректный базовый интерфейс. Указан пользовательский path: ' +
                     `${currentFeature.path}", но не указано имя фичи в поле name. Поправьте описание фичи в s3mod`,
                  moduleInfo
               });
               hasCriticalError = true;
            }

            this.interfaces.required.push(currentFeature);
         });
      ...
      if (moduleInfo.featuresProvided.length > 0) {
         moduleInfo.featuresProvided.forEach((providedFeature) => {
            const currentFeature = getFeatureMeta(providedFeature);


            if (currentFeature.path && !currentFeature.name) {
               logger.error({
                  message: 'Обнаружен некорректный провайдер. Указан пользовательский path: ' +
                     `${currentFeature.path}", но не указано имя фичи в поле name. Поправьте описание фичи в s3mod`,
                  moduleInfo
               });
               hasCriticalError = true;
            ...
            this.interfaces.providedOrder.push(currentFeature);
         });
      }
