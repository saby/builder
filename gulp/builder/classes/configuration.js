...
class BuildConfiguration extends BaseConfiguration {
   ...
   _configureModules(flagsList) {
      ...
      // for metatype module check if any dependant module has any changes
      this.modules.forEach((moduleInfo) => {
         // process metatype interface modules that has no changes since last build
         if (!moduleInfo.name.endsWith('-meta') || !moduleInfo.changedFiles) {
            return;
         }

         if (!moduleInfo.metaTsChanged) {
            moduleInfo.metaTsChanged = moduleInfo.fullDependsTree.some((moduleName) => {
               const currentModuleInfo = this.getModuleInfoByName(moduleName);

               if (!currentModuleInfo) {
                  return false;
               }

               return !currentModuleInfo.changedFiles ||
                  currentModuleInfo.changedFiles.length > 0 ||
                  currentModuleInfo.deletedFiles.length > 0;
            });
         }
      });
