...
const through = require('through2'),
   ...
   { isPrivate } = require('../../../lib/espree/library'),
   templateExtReg = /(\.tmpl|\.wml|\.js)$/;
const { useCompiledFiles } = require('../../../lib/helpers');
...
function checkForExternalPrivateDeps(moduleName, dependencies) {
   const result = [];
   dependencies
      .filter(dependencyName => isPrivate(dependencyName))
...
module.exports = function declarePlugin(taskParameters, moduleInfo) {
   ...
   return through.obj(async function onTransform(file, encoding, callback) {
      try {
         ...
         if (file.pBasename.endsWith('.html.tmpl')) {
            if (!taskParameters.htmlTmplFiles.hasOwnProperty(moduleInfo.outputName)) {
               taskParameters.htmlTmplFiles[moduleInfo.outputName] = [];
            }
            taskParameters.htmlTmplFiles[moduleInfo.outputName].push(file.pRelative);
         }
         ...
         const relativeSourcePath = path.relative(moduleInfo.path, file.pHistory[0]);
         if (taskParameters.config.extensionForTemplate === 'js') {
            // нужно передать в глобальное окружение, что шаблоны в .wml.js формате, чтобы
            // плагин wml при сборке html.tmpl отработал правильно и загрузил wml.js файл
            if (!global.contents) {
               global.contents = {
                  extensionForTemplate: 'js'
               };
            } else {
               global.contents.extensionForTemplate = 'js';
            }

            outputWmlJsFile = path.join(moduleInfo.output, transliterate(relativeSourcePath.replace(templateExtReg, '$1.js')));
            if (taskParameters.config.isReleaseMode) {
               outputWmlMinJsFile = path.join(moduleInfo.output, transliterate(relativeSourcePath.replace(templateExtReg, '$1.min.js')));
            }
         }

         if (taskParameters.config.isReleaseMode) {
            outputMinFile = path.join(moduleInfo.output, transliterate(relativeSourcePath.replace(templateExtReg, '.min$1')));

            if (taskParameters.config.sourceMaps) {
               outputMapFile = `${path.join(moduleInfo.output, transliterate(relativeSourcePath))}.map`;
         ...
         // заменяем повторяющиеся tw классы в исходном файле ПЕРЕД компиляцией
         if (moduleInfo.tailwindInfo && moduleInfo.tailwindInfo.replacers) {
            const modifiedSourceText = modifySourceText(
               file.contents.toString(),
               moduleInfo.tailwindInfo.replacers.selectors
            );

            file.contents = Buffer.from(modifiedSourceText);
         }

         // Write original file if tmpl can't be compiled
         let newText = file.contents.toString();

         const relativeFilePathWithModule = path.join(path.basename(moduleInfo.path), relativeSourcePath);
         const extension = file.pExtname.slice(1, file.pExtname.length);

         if (useCompiledFiles(taskParameters, moduleInfo)) {
            ...
            const [, result] = await execInPool(
               ...
               [
                  compiledPath,
                  taskParameters.cache.getCompiledHash(moduleInfo, relativeFilePathWithModule),
                  taskParameters.cache.getHash(moduleInfo, relativeFilePathWithModule)
               ],
               file.pHistory[0],
               moduleInfo
            );
            ...
            if (result) {
               ...
               moduleInfo.cache.storeBuildedMarkup(relativeFilePathWithModule, {
                  nodeName: `${extension}!${relativeFilePathWithModule.replace(file.pExtname, '')}`,
                  text: result
               });
         ...
         if (error) {
            ...
            logger.error({
               ...
               error,
               moduleInfo,
               filePath: relativeFilePathWithModule
            });
         } else {
            ...
            if (taskParameters.config.isReleaseMode) {
               ...
               const [errorUglify, obj] = await execInPool(
                  ...
                  relativeFilePathWithModule.replace(templateExtReg, '.min$1'),
                  moduleInfo
               );
               if (errorUglify) {
                  taskParameters.cache.markFileAsFailed(file.pRelativeSource(moduleInfo.path));
                  ...
                  logger.error({
                     ...
                     moduleInfo,
                     filePath: relativeSourcePath.replace(templateExtReg, '.min$1')
                  });
               } else {
            ...
            moduleInfo.cache.storeBuildedMarkup(relativeFilePathWithModule, result);
         }
