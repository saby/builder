...
const { componentCantBeParsed, moduleHasNoChanges, removeStrictMode } = require('../../../lib/helpers');
...
function execParseJsComponent(taskParameters, moduleInfo, parseOptions, fileContents, filePath) {
   return execInPool(
      taskParameters.pool,
      'parseJsComponent',
      [
...
function markFileAsFailed(taskParameters, moduleInfo, file, error) {
...
function modifyContentsWithTailwind(taskParameters, moduleInfo, contents) {
   const sourceWithDependency = modifyWithTailwind(
      contents.toString(),
   ...
   if (moduleInfo.tailwindInfo.replacers) {
      const sourceWithReplacements = modifySourceText(
         sourceWithDependency,
         moduleInfo.tailwindInfo.replacers.selectors
      );

      return Buffer.from(sourceWithReplacements);
   }

   return Buffer.from(sourceWithDependency);
}
...
module.exports = function declarePlugin(taskParameters, moduleInfo) {
   return through.obj(
      ...
      async function onTransform(file, encoding, callback) {
         ...
         if (file.useStrictFalse) {
            file.contents = removeStrictMode(file.contents);

            if (file.developmentContent) {
               file.developmentContent = removeStrictMode(file.developmentContent);
            }
         }
         ...
         let [error, meta] = await execParseJsComponent(
            taskParameters,
            moduleInfo,
            parseOptions,
            file.contents,
         ...
         if (error) {
            markFileAsFailed(taskParameters, moduleInfo, file, error);
            callback(null);
            return;
         }
         ...
         if (file.developmentContent) {
            ...
            [error, meta] = await execParseJsComponent(
               taskParameters,
               moduleInfo,
               parseOptions,
               file.developmentContent,
            ...
            if (meta.umdContent) {
               file.contents = Buffer.from(meta.umdContent);
            } else if (meta.amdContent) {
               file.contents = Buffer.from(meta.amdContent);
            } else {
         ...
         if (componentInfo) {
            if (taskParameters.config.isFacade({ path: componentInfo.componentName })) {
      ...
      function onFlush(callback) {
         ...
         try {
            ...
            if (
               ...
               taskParameters.cache.addOutputFile(moduleInfo.path, `${moduleInfo.outputName}/routes-info.json`, moduleInfo);
            }
         } catch (error) {
            getMetricsReporter().markFailedModule(moduleInfo);
            logger.error({
