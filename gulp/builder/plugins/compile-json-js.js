...
const LOCALIZATION_PATTERN = /lang\/\w{2}\/(\w{2})(-(\w{2}))?\.json/i;
   ...
   return through.obj(
      ...
      async function onTransform(file, encoding, callback) {
         ...
         try {
            ...
            const relativePath = path.relative(currentFileBase, file.pHistory[0]);
            const outputPath = path.join(moduleInfo.output, transliterate(relativePath.replace(/\.(json)$/, '.json.js')));
            const outputMinPath = outputPath.replace(/\.js$/, '.min.js');

            if (LOCALIZATION_PATTERN.test(relativePath)) {
               // const [,,, region] = LOCALIZATION_PATTERN.exec(relativePath);
