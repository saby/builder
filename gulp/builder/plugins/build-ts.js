...
const through = require('through2'),
   ...
   { shouldRemoveFromStream, getFacadeName, useCompiledFiles } = require('../../../lib/helpers'),
...
function shouldLoadTscEmit(file, taskParameters) {
   return (
      ...
      // в конфиге экспортируются типы для использования в ts-коде
      // модуля RequireJsLoader и tsc в результате оборачиваем результат
      // кода в дефайн в AMD и UMD формате, но именно конфиг require единственный
      // файл, который нужно скомпилить в js один в один как описано в исходном
      // коде. Единственный способ это сделать, собрать через typescript.transpileModule
      // с указанием compilerOptions.module = 'commonjs'
      // Это потребовалось, поскольку в project references не работает описание неймспейсов в
      // в отдельном .d.ts файле - типы нужно либо описывать в том же файле и экспортировать при
      // необходимости наружу либо импортить типы напрямую, и тот и другой способ вынудили
      // нас писать костыль и компилить конфиг через transpileModule
      !file.pPath.includes('RequireJsLoader/config.ts')
   );
}
...
async function loadTscEmittedFile(file, taskParameters, moduleInfo) {
   ...
   const [error, production] = await execInPool(
      ...
      'loadCompiledJs',
      [
         moduleInfo.typescriptOutputDir,
         relativeFilePath,
...
async function compileSingleFile(file, taskParameters, moduleInfo) {
      ...
      async function onTransform(file, encoding, callback) {
         try {
            ...
            if (file.pPath.endsWith('.meta.ts')) {
               moduleInfo.cache.addMetaTypesFile(`${moduleInfo.outputName}/${file.pRelative.replace(TS_EXT, '')}`);
            }
            ...
            if (
               useCompiledFiles(taskParameters, moduleInfo) &&
               !taskParameters.config.isFacade({ path: getFacadeName(moduleInfo, file) })
            ) {
            ...
            const isTscEmit = shouldLoadTscEmit(file, taskParameters);
            let result;

            if (isTscEmit) {
               result = await loadTscEmittedFile(file, taskParameters, moduleInfo);
            } else {
               result = await compileSingleFile(file, taskParameters, moduleInfo);
            }
            ...
            if (moduleInfo.name === 'WS.Core' && file.contents.toString().includes('// @ts-use-strict-false')) {
               jsFile.useStrictFalse = true;
            }
