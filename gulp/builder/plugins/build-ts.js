         ...
         try {
            ...
            if (isTscEmit) {
               try {
                  ...
                  // если файла нет, значит tsc его не скомпилировал, в таком случае запускаем изолированную компиляцию
                  // через typescript. Такая ситуация возможна, когда мы собирали через tsc только часть проекта.
                  if (err.code === 'ENOENT') {
                     result = await compileSingleFile(file, taskParameters, moduleInfo, sourceRoot);
                  } else {
                     throw err;
                  }
               }
