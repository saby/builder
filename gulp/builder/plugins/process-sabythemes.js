      ...
      async function onFlush(callback) {
         ...
         try {
            ...
            Object.keys(sabyThemes).forEach((currentTheme) => {
               ...
               taskParameters.addFilesToCopy('ThemesModule', [`${relativeOutputPathWoExt}.css`, `${relativeOutputPathWoExt}.json`]);
               if (taskParameters.config.minimize) {
                  ...
                  promises.push(fs.outputJson(`${outputPathWoExt}.min.json`, jsonResult));
                  taskParameters.addFilesToCopy('ThemesModule', [`${relativeOutputPathWoExt}.min.css`, `${relativeOutputPathWoExt}.min.json`]);
               }

               if (taskParameters.config.version) {
                  taskParameters.addVersionedModules('ThemesModule', [`ThemesModule/${relativeOutputPathWoExt}.css`]);

                  if (taskParameters.config.minimize) {
                     taskParameters.addVersionedModules('ThemesModule', [`ThemesModule/${relativeOutputPathWoExt}.min.css`]);
                  }
               }
