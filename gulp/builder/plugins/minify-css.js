      ...
      async function onTransform(file, encoding, callback) {
         try {
            ...
            if (file.productionContents) {
               newText = file.productionContents.toString();
            } else {
               newText = file.contents.toString();
            }
            ...
            const [error, minified] = await execInPool(
               ...
               [{
                  newMinimizer,
                  text: newText,
                  deepOptimize: deepOptimizeModules.has(moduleInfo.name),
            ...
            if (minified.errors.length > 0) {
               ...
               minified.errors.forEach((currentError) => {
                  logger.warning({
                     message: `Error while minifying css: ${currentError}`,
                     moduleInfo,
