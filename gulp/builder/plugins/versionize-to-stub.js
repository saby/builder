...
module.exports = function declarePlugin(taskParameters, moduleInfo) {
   const fontRoot = taskParameters.config.buildIE ? moduleInfo.path : '';
   const { cdnUrl } = taskParameters.config;
   const regions = taskParameters.config.countries;
   ...
   return through.obj(async function onTransform(file, encoding, callback) {
      ...
      try {
         ...
         if (file.pExtname === '.css') {
            result = await versionizeStyles(file, moduleInfo, { fontRoot, cdnUrl, regions });
         ...
         // will do it by himself with runtime generator parameters
            ...
            result = versionizeTemplates(file, moduleInfo, { cdnUrl, regions });
         }
