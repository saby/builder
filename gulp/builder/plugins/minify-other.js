   ...
   return through.obj(
      ...
      function onTransform(file, encoding, callback) {
         ...
         try {
            ...
            // Для словарей локализации не записываем *.min.json файлы
            const isJsonDict = (
               file.locDict &&
               currentExt === '.json' &&
               minFileExt === '.min.json'
            ...
            if (shouldIgnore(file) || isJsonDict) {
               callback(null, file);
               taskParameters.metrics.storePluginTime('minify other', startTime);
               return;
            }

            const relativePath = getRelativePath(moduleInfo.path, currentFilePath, moduleInfo.output);
            const outputMinFile = path.join(
               moduleInfo.output,
               transliterate(relativePath.replace(currentExt, minFileExt))
            );
