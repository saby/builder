...
module.exports = function declarePlugin(taskParameters, moduleInfo) {
   return through.obj(function onTransform(file, encoding, callback) {
      try {
         const shouldPushChangedFile = (
            GRANTED_EXTENSIONS.includes(file.pExtname) &&
            !NON_CACHED_META.includes(path.basename(file.pRelative)) &&
            !file.pBasename.endsWith(`.min${file.pExtname}`)
         );

         if (shouldPushChangedFile) {
