...
const { moduleHasNoChanges, sortObject } = require('../../../lib/helpers');
...
function getRegion(config, currentRegion) {
   const region = currentRegion.includes('-') ? currentRegion.split('-').pop() : undefined;

   if (FEATURE && config.rawConfig.countries && config.countries.includes(region)) {
      return region;
   }
...
function getRegionDictionaryPath(config, moduleInfo, currentRegion, locale) {
   const region = getRegion(config, currentRegion);
...
module.exports = function declarePlugin(taskParameters, moduleInfo) {
   DictionaryIndexer.setDictionaryIndexer(
      ...
      FEATURE ? taskParameters.config.rawConfig.countries : undefined,
            ...
            for (const locale of taskParameters.config.localizations) {
               ...
               if (mergedDictionary) {
                  Object.keys(mergedDictionary).forEach((currentRegion) => {
                     const region = getRegion(config, currentRegion);
                     const regionDictPath = getRegionDictionaryPath(config, moduleInfo, currentRegion, locale);

                     if (!FEATURE || !config.rawConfig.countries || config.countries.includes(region)) {
                        this.push(new PosixVinyl({
                           pBase: moduleInfo.output,
                           pPath: regionDictPath,
                           contents: Buffer.from(JSON.stringify(sortObject(mergedDictionary[currentRegion]))),
