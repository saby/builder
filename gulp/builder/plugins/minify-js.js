...
module.exports = function declarePlugin(taskParameters, moduleInfo) {
   return through.obj(
      ...
      async function onTransform(file, encoding, callback) {
         ...
         // сохраняем дебажную версию production-кода для tsx исходников,
         // поскольку при пересборке библиотек нам нужна дебажная версия
         // production-кода tsx файла, а получить её можно либо путём сохранения
         // в кеше, либо пересборкой конкретного tsx-файла
         if (file.productionContents) {
            await fs.outputFile(
               path.join(
               ...
               file.productionContents.toString()
            );
         }
