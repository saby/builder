...
class DependencyController {
   ...
   storeNode(nodeName, objectToStore, relativePath) {
      ...
      if (this.cfg.moduleInfo.name === 'WS.Core' && this.cfg.branchTests) {
         this.requireJsSubstitutions[`${nodeName}`] = toPosix(relativePath.replace('.ts', '.js'));
      }
   }
      ...
      async function onFlush(callback) {
         ...
         try {
            ...
            if (
               ...
               !helpers.moduleHasNoChanges(moduleInfo, [moduleInfo.moduleDependenciesChanged])
            ) {
         ...
         taskParameters.metrics.storePluginTime('presentation service meta - module-dependencies.json', startTime);
         callback();
      }
   );
}
