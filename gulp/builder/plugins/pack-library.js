      ...
      async function onFlush(callback) {
         ...
         await pMap(
            ...
            async(library) => {
               ...
               if (error) {
                  taskParameters.cache.markFileAsFailed(library.pHistory[0]);
                  logger.error({
                  ...
                  if (error.privateDependencies instanceof Array) {
                     error.privateDependencies.forEach(
                        dependency => taskParameters.cache.markFileAsFailed(path.join(moduleRoot, dependency))
                     );
                  }
               } else {
