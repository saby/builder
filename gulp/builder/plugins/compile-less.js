...
function fillRegionNodes(taskParameters, currentRegion, currentCssNode) {
   taskParameters.cache.addRegionNode(
      currentCssNode,
      currentRegion,
      '.icon-currentCurrency::before {\n' +
      '  content: "\\e6a3";\n' +
      '}',
      '.icon-currentCurrency::before {\n' +
      `  content: "${CURRENCY_GLYPH_MAP[currentRegion]['\\e6a3']}";\n` +
      '}'
   );
   taskParameters.cache.addRegionNode(
      currentCssNode,
      currentRegion,
      '.icon-currentCurrencyNone::before {\n' +
      '  content: "\\e9cd";\n' +
      '}',
      '.icon-currentCurrencyNone::before {\n' +
      `  content: "${CURRENCY_GLYPH_MAP[currentRegion]['\\e9cd']}";\n` +
      '}'
   );

   // надо прописать также минифицированный вариант, поскольку в дальнейшем замена будет производиться
   // в кастомных пакетах, которые пакуют минифицированные стили(или в особых случаях дебажные)
   taskParameters.cache.addRegionNode(
      currentCssNode,
      currentRegion,
      '.icon-currentCurrency::before{content:"\\e6a3"}',
      `.icon-currentCurrency::before{content:"${CURRENCY_GLYPH_MAP[currentRegion]['\\e6a3']}"}`
   );
   taskParameters.cache.addRegionNode(
      currentCssNode,
      currentRegion,
      '.icon-currentCurrencyNone::before{content:"\\e9cd"}',
      `.icon-currentCurrencyNone::before{content:"${CURRENCY_GLYPH_MAP[currentRegion]['\\e9cd']}"}`
   );
}
         ...
         try {
            ...
            if (file.cached) {
               ...
               taskParameters.config.countries.filter(currentRegion => currentRegion !== 'RU').forEach((region) => {
                  taskParameters.cache.migrateRegionNode(currentCssNode, region);
               });
                  ...
                  async(currentRegion) => {
                     ...
                     if (currentRegion !== 'RU') {
                        ...
                        if (
                           ...
                           fillRegionNodes(taskParameters, currentRegion, currentCssNode);

                           const currentRegionNodes = taskParameters.cache.getRegionNode(currentCssNode, currentRegion);
