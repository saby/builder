...
function getIconSizeByViewBox(viewBox) {
   ...
   if (viewBox) {
      ...
      if (upperRightX === upperRightY && ICON_DIMENSIONS[upperRightX]) {
         iconPostfix = ICON_DIMENSIONS[upperRightX];
      }
   }
   return iconPostfix;
}
...
function processSvg(taskParameters, moduleInfo) {
   ...
   return through.obj(
      ...
      function onFlush(callback) {
         const noSvgChanges = helpers.moduleHasNoChanges(moduleInfo, [moduleInfo.svgChanged]);

         // если svg-файлы в данном модуле не менялись и нам не нужны данные из кеша для сохранения
         // корневой меты icons.json, пропускаем этот этап
         if (noSvgChanges && !taskParameters.config.joinedMeta) {
            callback(null);
            return;
         }
