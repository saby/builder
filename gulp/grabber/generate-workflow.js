...
const guardSingleProcess = require('../common/generate-task/guard-single-process.js'),
   ...
   { processFilesWithES5Extension } = require('../../lib/helpers');
...
function generateTaskForES5Types(taskParameters) {
   return function processES5ExtensionFiles() {
...
function generateWorkflow(processArgv) {
   ...
   return gulp.series(
      ...
      generateTaskForES5Types(taskParameters),
...
function generateTaskForSaveOutputJson(taskParameters) {
   return async function saveOutputJson() {
      await fs.outputJSON(taskParameters.config.outputPath, taskParameters.cache.getCachedWords(), { spaces: 1 });
   };
}
