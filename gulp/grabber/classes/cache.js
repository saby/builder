...
class Cache {
    ...
    * @returns {boolean}
    */
   isFileChanged(filePath, fileContents, hashByContent, fileTimeStamp, moduleInfo) {
      // папки не меняются
      if (!fileContents) {
         return false;
      }
      const prettyPath = toSafePosix(filePath);
      const prettyRelativePath = transliterate(getRelativePath(moduleInfo.appRoot, prettyPath));
      const hash = crypto.createHash('sha1').update(fileContents).digest('base64');
      const isChanged = this._isFileChanged(prettyRelativePath, hash);

      if (!isChanged) {
         this.currentStore.cachedFiles[prettyRelativePath] = this.lastStore.cachedFiles[prettyRelativePath];
      } else {
         this.currentStore.cachedFiles[prettyRelativePath] = {
            words: [],
            hash
         };
      }
