...
function getResourceRoot(config) {
   // Во всех приложениях, кроме десктопных и собранных через wasaby-cli,
   // используется сервис статики для помодульных ресурсов.
   // P.S. а также сервиса статики нет в приложениях, где указан cdnUrl, отличный от /cdn/,
   // например некоторые приложения облака, которые живут чисто на сервисе представлений
   // FIXME: https://online.sbis.ru/opendoc.html?guid=93c58bc0-918d-4f59-aa79-3867b252ea4e&client=3
   if (!config.desktop && !config.joinedMeta && (!config.cdnUrl || config.cdnUrl === '/cdn/')) {
      return '/static/resources/';
   }

   if (config.resourcesUrl) {
      return `${config.applicationForRebase}resources/`;
   }

   return config.applicationForRebase;
}
...
async function saveThemes(taskParameters, root, isThemeForReleaseOnly, fileSuffix) {
   ...
   const resourceRoot = getResourceRoot(taskParameters.config);
...
module.exports = function generateTaskForSaveJoinedMeta(taskParameters) {
   ...
   return async function saveJoinedMeta() {
      ...
      if (taskParameters.config.contents) {
         ...
         if (commonContents) {
            commonContents.modules = sortObject(commonContents.modules);
