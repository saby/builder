...
function generateTaskForGenerateJson(taskParameters) {
   ...
   return async function generateJson() {
      ...
      // если js-код не менялся и кеш сборки не сбрасывался, нет смысла запускать пересборку
      // метаданных components-properties.json
      if (!taskParameters.config.jsChanged && !getBuildStatusStorage().cacheIsDropped) {
