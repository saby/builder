...
class TaskParameters {
   ...
   addIntoInterfaceRoute(currentInterface, filePath) {
      if (!this.interfaceRoute[currentInterface]) {
         this.interfaceRoute[currentInterface] = [];
      }
      this.interfaceRoute[currentInterface].push(filePath);
   }

   getInterfaceRoute() {
      Object.keys(this.interfaceRoute).forEach((currentInterface) => {
         this.interfaceRoute[currentInterface] = this.interfaceRoute[currentInterface].sort();
      });
   ...
   addFileToCopy(moduleName, relativePath) {
      if (!this.filesToCopy.hasOwnProperty(moduleName)) {
         this.filesToCopy[moduleName] = new Set();
      }
      this.filesToCopy[moduleName].add(relativePath);
   }
   ...
   addVersionedModule(moduleName, fileName) {
      if (!this.versionedModules[moduleName]) {
         this.versionedModules[moduleName] = [];
      }
      if (!this.versionedModules[moduleName].includes(fileName)) {
         this.versionedModules[moduleName].push(fileName);
      }
   }
