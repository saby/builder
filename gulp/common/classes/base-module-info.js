...
class ModuleInfo {
   constructor(baseModuleInfo) {
      ...
      this.outputName = transliterate(path.basename(this.path));
      if (changedFiles) {
         this.setDefaultChangedFiles();
         changedFiles.forEach((currentFile) => {
            this.checkForCommonExtensions(currentFile, 'change');
      ...
      if (deletedFiles) {
         deletedFiles.forEach((currentFile) => {
            this.checkForCommonExtensions(currentFile, 'delete');
   ...
   checkForRoutesComponent(filePath, extension, type) {
      if (filePath.endsWith(`.routes${extension}`)) {
         this.routesFilesChanged = true;

         if (type === 'delete') {
            this.addFileToList('deletedFiles', 'routes-info.json');
         }
      }
   }
   ...
   checkForCommonExtensions(currentFile, type) {
      const prettifiedPath = toPosix(currentFile);
      const extname = path.extname(prettifiedPath);

      switch (extname) {
         case '.ts':
         case '.tsx':
            this.typescriptChanged = true;
            this.moduleDependenciesChanged = true;
            if (prettifiedPath.endsWith('.meta.ts')) {
               this.metaTypesChanged = true;
            }

            this.checkForRoutesComponent(prettifiedPath, extname, type);
            break;

         // все нижеописанные расширения влияют на мета-данные module-dependencies.json
         case '.wml':
         case '.xhtml':
         case '.tmpl':
         case '.json':
         case '.jstpl':
            ...
            if (prettifiedPath.endsWith('.html.tmpl')) {
         ...
         case '.css':
            ...
            if (path.basename(prettifiedPath) === 'theme.less') {
      ...
      if (prettifiedPath.startsWith('lang/') && (prettifiedPath.endsWith('.json') || prettifiedPath.endsWith('.css'))) {
         if (type === 'delete') {
            this.dropLocalizationCache = true;
         }
   ...
   get nameWithResponsible() {
      const responsible = this.responsible || (this.kaizen && this.kaizen.responsible);
   ...
   symlinkInputPathToAvoidProblems(cachePath, buildTask, watcherRunning) {
      ...
      if (needSymlink) {
         ...
         // для WS.Core запишем прямой путь до исходников, нужен для передачи в tsconfig
         if (path.basename(newPath) === 'WS.Core') {
