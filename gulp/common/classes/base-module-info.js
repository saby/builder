...
class ModuleInfo {
   constructor(baseModuleInfo) {
      ...
      if (changedFiles) {
         ...
         changedFiles.forEach((currentFile) => {
            ...
            if (currentFile.endsWith('.meta.ts')) {
   ...
   symlinkInputPathToAvoidProblems(cachePath, buildTask, watcherRunning) {
      ...
      if (needSymlink) {
         ...
         // а для Controls-default-theme перед сборкой less необходимо подменить content
         ...
         //  будет удалить
         if (['Types', 'Controls-default-theme'].includes(path.basename(newPath))) {
            if (watcherRunning) {
