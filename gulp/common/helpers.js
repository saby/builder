...
/**
 * Количество последних директорий в логах, которое необходимо сохранять.
 * @type {number}
 */
const LOGS_SIZE_TO_KEEP = 10;
...
function needSymlink(taskParameters, config, moduleInfo, isFirstBuild) {
   ...
   const checkForSymlink = (file) => {
      taskParameters.addFilesToCopy(moduleInfo.outputName, [file.pRelative]);
...
function generateTaskForLoadCache(taskParameters) {
   return async function loadCache() {
      ...
      const lockFilePath = withLockfile.toFileName(taskParameters.config.cachePath, 'common-cache');

      await withLockfile(lockFilePath, async() => {
         if (taskParameters.config.compiled) {
            await taskParameters.cache.loadCompiled(taskParameters.config.modules);
         }

         await taskParameters.cache.load(changedFilesWithDependencies, taskParameters.config.watcherRunning);
      });

      getMetricsReporter().builderCacheExists = !taskParameters.cache.isFirstBuild();
...
function generateTaskForInitWorkerPool(taskParameters, rootForPlatform) {
   return async function initWorkerPool() {
      ...
      if (global.contents) {
         process.env['global-contents'] = JSON.stringify(global.contents);
      }
...
async function saveExtraMetaIntoLogs(taskParameters) {
   const { logs, startBuildTime } = taskParameters.config;
   ...
   if (taskParameters.config.builderTests) {
      ...
      currentBuildLogsPath = path.join(logs, `${startBuildTime}`);
   }

   await fs.outputJson(`${currentBuildLogsPath}/garbage.json`, taskParameters.config.getGarbageList());
}

async function removeOutdatedLogs(taskParameters) {
   if (taskParameters.config.builderTests) {
      return;
   }

   const logsContents = await fs.readdir(taskParameters.config.logs, {
      withFileTypes: true,
      recursive: false
   });

   const directories = logsContents
      .filter(dirEntry => dirEntry.isDirectory() && !Number.isNaN(Number.parseInt(dirEntry.name, 10)))
      .map(dirEntry => Number.parseInt(dirEntry.name, 10))
      .sort((a, b) => a - b);

   if (directories.length > LOGS_SIZE_TO_KEEP) {
      const directoriesToRemove = directories.slice(0, directories.length - LOGS_SIZE_TO_KEEP);

      await Promise.all(
         directoriesToRemove.map(name => fs.promises.rm(path.join(taskParameters.config.logs, `${name}`), {
            recursive: true,
            force: true
         }))
      );
...
function generateTaskForSaveCache(taskParameters, migrateCache) {
   return async function saveCache() {
      ...
      await withLockfile(lockFilePath, async() => {
         ...
         await removeOutdatedLogs(taskParameters);

         // сохраняем информацию о moduleInputs для каждого интерфейсного модуля.
         const moduleInputsMeta = taskParameters.getModuleInputsMeta();
         await fs.outputJson(
            path.join(taskParameters.config.cachePath, 'module-inputs.json'),
            moduleInputsMeta
         );
         await fs.outputJson(
            path.join(taskParameters.config.logFolder, 'module-inputs.json'),
            moduleInputsMeta
         );
      });
