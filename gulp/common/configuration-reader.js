...
function getProcessParameters(argv) {
   ...
   for (const argument of argv) {
      // Временный фикс работы watcher'а для локальных стендов. На никсах jinnee экранирует кавычки и при передаче
      // через несколько процессов происходит двойное экранирование и получается --config="\"path\""
      // что при запуске watcher ломает парсинг аргументов командной строки.
      // TODO сделать постоянное решение по задаче
      //  https://online.sbis.ru/opendoc.html?guid=90da4f87-18bb-4176-b9a6-23c355d38aa4&client=3
      const normalizedArgument = argument.replace(/"\\"|\\""/g, '"');
      const match = normalizedArgument.match(/^--([^=]+)=['"]?([^'"]*)['"]?$/i);
      if (match) {
