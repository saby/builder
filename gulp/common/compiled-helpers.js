 ...
 * @param {boolean} needToSymlinkCompiled select whether compiled module should be skipped or
 * created symlink to compiled. needed for tasks, that works with already built or copied interface module
 * @param {boolean} onlyChangedModules build only modules which have changed files
 */
function getTasksTypesByModules(modules, needToSymlinkCompiled, onlyChangedModules) {
   const meta = {
      skip: [],
      build: [],
      symlink: []
   ...
   for (const moduleInfo of modules) {
      ...
      } else if (onlyChangedModules) {
         if (Array.isArray(moduleInfo.changedFiles) && moduleInfo.changedFiles.length > 0) {
            meta.build.push(moduleInfo);

            // Watcher provides a list of changed files, there is no need to calculate hash-sum.
