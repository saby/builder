...
try {
   ...
   async function initializeWSForWorker() {
      // ws должен быть вызван раньше чем первый global.requirejs
      const wsEnvironment = require('../../lib/wasaby/init');

      const applicationRoot = process.env['application-root'];
      const resourcesPath = process.env['resources-path'];
      const contents = process.env['global-contents']
         ? JSON.parse(process.env['global-contents'])
         : undefined;
      const requiredModules = JSON.parse(process.env['required-modules']);

      await wsEnvironment.init(applicationRoot, resourcesPath, contents, requiredModules);
   }

   process.on('unhandledRejection', (reason, p) => {
      const error = {
         message: `worker's critical error. Unhandled promise rejection at:\n ${p}\nreason:\n ${reason.message} \n ${reason.stack}`
      };

      fs.outputJsonSync(path.join(process.env.logsPath, `worker/worker-critical-error-${process.pid}-reason.json`), {
   ...
   const
      ...
      sabyPageProcessor = require('../../lib/pagex/load-modules'),
      ...
      generateTailwind = require('../../lib/tailwind/generate'),
      analyzeProjectDependencies = require('../../lib/analyze-dependencies');
    ...
    */
   async function buildTmpl(text, relativeFilePath, componentsPropertiesFilePath, templateOptions) {
      const startTime = Date.now();
      if (!processingTmpl) {
         await initializeWSForWorker();
   ...
   async function buildHtmlTmpl(
      ...
      try {
         // При выполнении построения шаблона для корректной работы UI генераторов необходимо
         // наличие process.domain, который используется в проверках совместимости.
         // На СП свойство присутствует и используется, при сборке этого свойства нет, создадим на его месте объект.
         if (!process.domain) {
            process.domain = { };
         }
      } catch (e) {
   ...
   async function compress(data) {
      ...
      const [gzippedContent, brotliContent] = await Promise.all([
         gzip(dataBuffer),
         brotli(dataBuffer)
      ]);
   ...
   function asyncRequireFile(filePath) {
      return new Promise((resolve) => {
         global.requirejs([filePath], result => resolve({ result }), error => resolve({ error }));
      });
   }

   async function metaTsToJson(filePath) {
      if (!global.requirejs) {
         await initializeWSForWorker();
      }

      // do async require of module to process require errors
      // properly and avoid unexpected terminate of builder process
      // caused by broken dependency during require of current metafile
      const { result, error } = await asyncRequireFile(filePath);

      if (error) {
         return { error };
      }

      if (result.default && typeof result.default.saveMeta === 'function') {
         return { result: result.default.saveMeta(true) };
      }

      if (typeof result.saveMeta === 'function') {
         return { result: result.saveMeta(true) };
      }
   ...
   async function doAsyncFs(operation, from, to, logOperation) {
      ...
      if (logOperation) {
         logger.info(`${operation} completed from "${from}" into "${to}"`);
      }
   }
   ...
   function minifyCss(options) {
      // По какой-то причине именно при работе сбора фраз локализации падает
      // ошибка с кодом 1011(Segmentation Fault) и только под CentOS 7(на RHEL 9
      // пока проверить нет возможности, поскольку ещё не установлена, нужно будет
      // проверить на ней)
      // Опытным путём выяснил, что проблема в новой библиотеке lightningcss, при
      // убивании пула воркеров процесс зависает и выбрасывает данную ошибку, обновление
      // lightningcss до актуальной версии не помогает исправить ошибку, есть пара issue
      // на этот счёт
      // https://github.com/parcel-bundler/lightningcss/issues/590
      // https://github.com/parcel-bundler/lightningcss/issues/611
      // но им уже больше полугода и когда они будут исправлены неизвестно, а пока будем
      // инициализировать минификацию css по востребованию
      // TODO разобраться с данной проблемой
      if (!runMinifyCss) {
         runMinifyCss = require('../../lib/run-minify-css');
      }

      return runMinifyCss(options);
   }

   workerPool.worker({
      getSabyPageLayouts: wrapWorkerFunction(async() => {
         ...
         return sabyPageProcessor.getLayoutModules();
      }),
      getSabyPageContents: wrapWorkerFunction(async(filePath, contents) => {
         // TODO: выполнить рефакторинг
         if (!global.requirejs) {
            await initializeWSForWorker();
         }
         ...
         return sabyPageProcessor.getContentModules(filePath, contents);
      }),
      ...
      minifyCss: wrapWorkerFunction(minifyCss),
      ...
      generateTailwind: wrapWorkerFunction(generateTailwind),
      analyzeProjectDependencies: wrapWorkerFunction(analyzeProjectDependencies)
