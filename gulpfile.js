...
try {
   ...
   const metricsReporter = require('./gulp/common/classes/metrics-reporter')();
   const hooks = require('./gulp/common/classes/hooks').hooks();
   const { buildInfoExitHandler } = require('./gulp/common/classes/build-info');
   ...
   process.on('exit', (resultCode) => {
      ...
      metricsReporter.save(cacheFolder);
      metricsReporter.save(logFolder);
      buildInfoExitHandler(cacheFolder);
   ...
   if (process.argv.includes('buildOnChange')) {
      ...
      const generateBuildOnChangeWatcher = require('./gulp/builder/generate-watcher-new');
      gulp.task('buildOnChangeWatcher', generateBuildOnChangeWatcher);
